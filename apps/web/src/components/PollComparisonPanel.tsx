  ...
  return (
    ...
    >
      <div className="space-y-4">
        <div>
          <div>
            <p className="text-xs uppercase tracking-[0.22em] text-accent">{t('poll.eyebrow')}</p>
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_auto_1fr] lg:items-stretch">
          {comparison.leadingCandidates.slice(0, 1).map((candidate) => (
            <article key={candidate.candidateId} className="pixel-corners border border-sky-400/30 bg-bg/35 p-4">
              ...
              <dl className="mt-4 grid gap-2 text-sm text-slate-300">
                <div className="flex justify-between gap-3"><dt className="text-slate-500">{t('poll.support')}</dt><dd>{candidate.supportPercent}%</dd></div>
                <div className="flex justify-between gap-3"><dt className="text-slate-500">{t('poll.trend')}</dt><dd>{candidate.trendLabel}</dd></div>
              </dl>
            </article>
          ))}
          ...
          <div className="pixel-corners flex items-center justify-center border border-line/70 bg-panelAlt/45 px-4 py-6 text-center">
            <div>
              ...
              <p className="mt-2 text-xs text-slate-400">{t('poll.comparison')}</p>
            </div>
          </div>
        ...
        <div className="pixel-corners border border-line/70 bg-bg/35 p-4">
          <p className="text-xs uppercase tracking-[0.22em] text-slate-500">{t('poll.otherCandidates')}</p>
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            {comparison.otherCandidates.map((candidate) => (
              <div key={candidate.candidateId} className="pixel-corners border border-line/60 bg-panelAlt/35 p-3">
                ...
                <p className="mt-2 text-sm text-slate-300">{t('poll.support')} {candidate.supportPercent}%</p>
              </div>
            ))}
          </div>
        </div>
