...
2026-09-06 依[中選會全國公告索引](https://web.cec.gov.tw/central/article/64374)核對 22 縣市來源頁、附件網址、公告日期與 SHA-256。共用匯入器目前可嚴格驗證並同步 15 縣市；來源含 9 份 ODS 及 6 份文字型 PDF。PDF 以固定欄位座標擷取，並要求投票所編號自 0001 完整連號，再通過行政區、村里、地址、鄰號和重複分派檢查。
...
| 項目 | 15 縣市合計 |
|---|---:|
| 一般選舉人投開票所／村里分派列 | 14,748 |
| 涵蓋村里 | 6,082 |
| 明確鄰號對應 | 94,953 |
| 無法只靠鄰號唯一判定 | 117 |

新竹市官方 PDF 有 359 個連續投票所編號，其中 344 筆有一般選舉人村里分派；嘉義縣有 540 個連續投票所編號，其中 513 筆有一般選舉人村里分派。其餘列只有其他選舉人類別，不混入一般戶籍地區查詢。

合計 117 筆標示 ambiguous；相關村里不會僅憑鄰號宣稱唯一對應。其中新北 23 筆路段、門牌或戶籍條件保留官方原文，苗栗與南投各 2 筆是官方同鄰分流，臺北、桃園、臺中及新竹市共 90 筆是 PDF 跨欄或門牌條件，均只列可能場所。其餘重複鄰號、全里與其他場所衝突、重複場所列、超過查詢上限等情形會使匯入失敗。
...
- 投開票所解析、鄰號正規化與快照替換測試 10 通過；CI 已新增根目錄資料工具工作。
- 15 縣市 dry run 與本機套用皆通過；實際寫入 14,748 筆分派與 94,953 筆鄰別。另以暫時第 999 鄰驗證同來源重匯後殘留為 0。
