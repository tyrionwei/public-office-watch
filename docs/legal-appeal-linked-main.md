...
上述回填沒有更改既有 review status、公開旗標、程序狀態或其他 claim 欄位；正式與 rehearsal
寫入仍為 0。41 筆舊案加上先前指定的單筆上訴案，共 42 筆已整理為
`20260922103804_backfill_reviewed_legal_judgment_dispositions.sql`，預定在正式 migration 時一併帶入。
migration 只合併 `judgmentDisposition`，並在 claim、人物、來源、審核／公開狀態或既有主文資料
衝突時停止；6 筆受阻案未納入。首次套用與重複套用已在 full local 以交易回滾驗證，持久寫入為 0。
此摘要不表示受阻或部分整理案件已完成，也不擴張目前僅支援單層、逐人全部上訴駁回的範圍。
