...
- 集中候選人頁：`https://youth.pfpnext.com/2026/`，2026-07-29 初次擷取為 7 位，
  2026-08-11 每週監測時已增加為 9 位縣市議員參選人。
...
- 目前親民黨 7 筆已有 5 筆完成身分確認；劉成謙、蕭夙玲 2 筆新人物仍維持本機待審核。
  B 級、同名衝突及新人物均不自動決定。
- 2026-08-11 新增黃舒琳（基隆市第 5 選舉區）與鄭素禎（基隆市第 2 選舉區）；
  兩筆均已唯一匹配 2026 選區並寫入本機待審區，身分判定為 `new_person`。
  最新 9 筆來源快照保存於
  `data-sources/pfp/2026-election/normalized-candidates-2026-08-11.json`，尚未建立人物、候選關係或公開資料。
...
- 使用者完成其餘身分審核後，另確認 1 筆既有人物；剩餘 86 筆均沒有可用的既有人物候選，
...
- 2026-08-01 依人工審核確認，高雄市第 11 選區張耀中與既有台中市議員張耀中不是同一人。
  已建立來源限定的獨立私有人物與 `potential` 候選關係，並留下「不合併」決定；由於原始民進黨網頁 key
  可能過期或錯誤，人物、候選關係與聲明均不公開，等中選會公告後再比對。
...
- 2026 政黨候選人的本機身分審核佇列已清空；317 筆候選關係及聲明已進入本機公開預覽，
  ...
  另有 1 筆張耀中來源已建立獨立私有人物及 `potential` 候選關係，等待中選會確認。
...
- 發布前固定驗證 317 筆有效候選，以及 1 筆已有獨立人物、但等待中選會確認的私有來源，
  並檢查人物／選區／聲明一致，阻擋同一 canonical person 被連到多筆 2026 候選關係。
- 寫入後先刷新 `public_people_list_cached`，再執行 `published.promote`，避免新人物出現在候選清單卻無法開啟人物頁。
- 同一流程會把已通過身分審核、且官方快照確實提供的個人欄位轉為 verified public claims；
  目前共發布學歷 65 筆、經歷 69 筆、政見 70 筆。人物頁優先顯示這類結構化官方履歷，
  避免既有概略欄位遮住較完整的新資料。
- 民進黨與國民黨本批集中公告未提供候選人的學歷、經歷或政見，因此不推測、不由其他文字自動補值；
  後續需在人物審核後另找候選人官方頁面或選舉公報補齊。
- 已驗證新北市與臺北市同名張志豪分別連到正確人物；等待中選會確認的高雄張耀中維持私有，
  不會出現在公開選區頁。其他新建立的候選人人物頁、搜尋及政黨提名標示均可正常顯示。
- 這一步只變更本機資料庫；正式站尚未發布。
...
- 發布預覽仍為 317 筆有效候選；張耀中來源另以私有人物與 `potential` 候選關係保留，
  等待中選會確認。這一步只產生 `local-data/party-candidate-freshness-2026-07-30/report.json`，沒有寫入資料庫。
