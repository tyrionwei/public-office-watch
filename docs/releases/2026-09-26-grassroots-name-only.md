...
### 2026-09-27 調整為每10批完整核對

使用者將完整逐表檢查頻率改為每10批。從已核對的前27批起，檢查點為37、47、57、60；完整8表及public/canonical多重集合契約不變，未通過檢查點不得進入下一組。每批仍有原5分鐘整交易及statement逾時、5秒鎖定、22表鎖、容量、提交紀錄與必要target/link檢查；原3條DML內容及順序不變，直接執行以檢查受影響筆數。

刪除前仍確認FK/race、未支援依賴、候選與facts的逐ID連結、merge列及會被連帶刪除資料的主鍵全集，避免未封存的新列在連帶刪除後無從察覺。非關鍵欄位完整比對移到檢查點，期間依賴既有全站及DB停寫；不得宣稱每批已完整逐列驗證。
