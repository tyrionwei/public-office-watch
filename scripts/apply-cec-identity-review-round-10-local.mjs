...
assert(args.every(x=>x==='--apply-local'), 'Usage: node scripts/apply-cec-identity-review-round-10-local.mjs [--apply-local]');
const apply=args.includes('--apply-local');
const base=path.resolve('tmp/cec-registration-identity-review-all');
const archive=path.resolve('tmp/cec-registration-final');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,''));
const sha=x=>createHash('sha256').update(x).digest('hex');
const quote=x=>"'"+x.replaceAll("'","''")+"'";
const json=x=>quote(JSON.stringify(x))+'::jsonb';
const ledger=read('docs/cec-registration-identity-review-2026-09-05-round-10.json');
...
assert.equal(audit.length,1156);assert.equal(new Set(audit.map(x=>x.claimId)).size,1156);
...
assert.equal(review.environment,'full-local');assert.equal(review.records.length,8);
...
assert.equal(new Set(accepted.map(r=>r.personId)).size,8);
assert.equal(new Set(review.records.map(r=>r.claimId)).size,8);
...
for(const r of accepted){
 ...
 const gender=evidence.claims.find(x=>r.evidenceClaimIds.includes(x.id)&&x.claim_type==='gender'&&x.review_status==='verified');
 assert(gender&&normalizeGender(gender.claim_value)===normalizeGender(r.registrationGender));
 const historical=evidence.candidates.find(x=>r.evidenceCandidateIds.includes(x.id));
 assert.equal(historical.canonical_person_id,r.personId);assert.equal(historical.year,2022);assert(historical.is_public);
 assert(evidence.people.some(p=>p.canonical_person_id===r.personId&&nameKey(p.name)===nameKey(r.name)));
 assert(historical.title.startsWith(r.locality??r.district));
 if(r.reasonCode==='normalized_full_name_gender_party_2022_local_history')assert.equal(canonicalPartyName(historical.party),canonicalPartyName(r.registrationParty==='無'?'無黨籍':r.registrationParty));
 else {assert.equal(r.name,'張景舜');assert.equal(historical.is_elected,true);assert(r.externalEvidence.statement&&r.evidenceUrls.includes(r.externalEvidence.url));}
 if(r.registrationBirth){const year=Number(r.registrationBirth.slice(0,3))+1911;assert(evidence.claims.some(c=>r.evidenceClaimIds.includes(c.id)&&c.claim_type==='birth_date'&&Number(c.claim_value.slice(0,4))===year));}
}
...
const sql=`BEGIN;
SET LOCAL application_name='cec-identity-review-round-10-20260905';
...
DO $check$ BEGIN
 ...
 IF (SELECT count(*) FROM claims_before)<>8 OR (SELECT count(*) FROM sources_before)<>8 THEN RAISE EXCEPTION 'Incomplete pending review inputs'; END IF;
...
SELECT id,source_person_id,person_id,'auto_matched',100,'cec_registration_identity_review_v8',match_reason,evidence_json,reviewed_by,reviewed_at,reviewed_at FROM new_matches;
UPDATE public.person_claims cl SET person_id=CASE WHEN i.decision='pending' THEN cl.person_id ELSE i.person_id END,
 ...
 claim_json=cl.claim_json || jsonb_build_object('identityReviewRound10',i.review),
 scoring_version='cec-registration-identity-review-v11',scoring_reasons=coalesce(cl.scoring_reasons,'[]'::jsonb) || jsonb_build_array(jsonb_build_object('reason',i.review->>'reason','reasonCode',i.review->>'reasonCode','reviewedAt',i.review->>'reviewedAt')),updated_at=now()
...
DO $verify$ BEGIN
 IF (SELECT count(*) FROM new_candidates n JOIN published.candidates c ON c.candidate_id=n.id AND c.person_id=n.person_id AND c.race_id=n.race_id AND c.candidacy_status='registered' AND c.registration_status='registered' AND c.election_result='pending' AND c.candidate_no IS NULL)<>8
 THEN RAISE EXCEPTION 'Published candidate parity failed'; END IF;
 IF (SELECT count(*) FROM new_events n CROSS JOIN LATERAL published.candidate_lifecycle_for(n.candidate_id) ev WHERE ev.event_type='registration_filed' AND ev.occurred_on IS NOT DISTINCT FROM n.occurred_on AND ev.source_url=n.source_url AND ev.candidate_no IS NULL)<>8
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>1148
 THEN RAISE EXCEPTION 'Remaining pending count failed'; END IF;
END; $verify$;
UPDATE public.person_claims cl SET claim_json=cl.claim_json || jsonb_build_object('identityReviewRound10Audit',a.review),updated_at=now() FROM audit_input a WHERE cl.id=a.claim_id;
DO $audit_verify$ BEGIN
 IF (SELECT count(*) FROM audit_input a JOIN public.person_claims c ON c.id=a.claim_id WHERE c.claim_json->'identityReviewRound10Audit'=a.review AND NOT c.is_public AND c.visibility='review_only' AND c.claim_json->'registrationEvidence'=a.expected->'claim_json'->'registrationEvidence')<>1156 THEN RAISE EXCEPTION 'Comprehensive audit coverage or private boundary failed';END IF;
