...
assert(args.every(x=>x==='--apply-local'), 'Usage: node scripts/apply-cec-identity-review-round-11-local.mjs [--apply-local]');
const apply=args.includes('--apply-local');
const base=path.resolve('tmp/cec-registration-identity-review-11');
const archive=path.resolve('tmp/cec-registration-final');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,''));
const sha=x=>createHash('sha256').update(x).digest('hex');
const quote=x=>"'"+x.replaceAll("'","''")+"'";
const json=x=>quote(JSON.stringify(x))+'::jsonb';
const ledger=read('docs/cec-registration-identity-review-2026-09-05-round-11.json');
...
assert.equal(audit.length,1148);assert.equal(new Set(audit.map(x=>x.claimId)).size,1148);
...
assert.equal(review.environment,'full-local');assert.equal(review.records.length,27);
...
assert.equal(new Set(accepted.map(r=>r.personId)).size,27);
assert.equal(new Set(review.records.map(r=>r.claimId)).size,27);
...
const scope=s=>String(s??'').normalize('NFKC').replaceAll('台','臺').replace(/\s/g,'').replace(/\((?:直轄市|縣市)\)/g,'').replace(/(?:鄉民代表|鎮民代表|市民代表|區民代表|村長|里長|鄉長|鎮長|市長|區長)選舉$/,'').replace(/第0+(\d+)選舉區/,'第$1選舉區');
for(const r of accepted){
 ...
 assert(evidence.people.some(p=>p.canonical_person_id===r.personId&&nameKey(p.name)===nameKey(r.name)));
 const histories=evidence.candidates.filter(c=>r.evidenceCandidateIds.includes(c.id));assert(histories.length);
 for(const h of histories){assert.equal(h.canonical_person_id,r.personId);assert(h.is_public&&h.year>=2018&&h.year<=2022);}
 const sex=normalizeGender(r.registrationGender);assert(p.genders.length<=1);if(sex)assert(p.genders.every(g=>g===sex));
 if(r.reasonCode==='same_name_exact_local_district_no_identity_conflict'){
  assert(['village_chief','township_representative','indigenous_district_representative'].includes(r.office));
  assert(histories.every(h=>scope(h.title)===scope(r.district)));assert(p.exactLocal.length);
 }else{
  assert.equal(r.reasonCode,'same_name_birth_year_county_history_unique');
  const m=r.registrationBirth.match(/^(\d{3})\/(?:\*\*|\d{2})\/(?:\*\*|\d{2})$/);assert(m);
  assert.deepEqual(p.birthYears,[Number(m[1])+1911]);assert(histories.every(h=>scope(h.title).startsWith(scope(r.county))));
  assert(evidence.claims.some(c=>r.evidenceClaimIds.includes(c.id)&&['birth_date','birth_year'].includes(c.claim_type)&&Number(c.claim_value.slice(0,4))===p.birthYears[0]));
 }
 for(const id of r.evidenceClaimIds){const claim=evidence.claims.find(c=>c.id===id);assert(claim&&claim.canonical_person_id===r.personId&&claim.review_status==='verified');}
}
...
const sql=`BEGIN;
SET LOCAL application_name='cec-identity-review-round-11-20260905';
...
DO $check$ BEGIN
 ...
 IF (SELECT count(*) FROM claims_before)<>27 OR (SELECT count(*) FROM sources_before)<>27 THEN RAISE EXCEPTION 'Incomplete pending review inputs'; END IF;
...
SELECT id,source_person_id,person_id,'auto_matched',100,'cec_registration_identity_review_v9',match_reason,evidence_json,reviewed_by,reviewed_at,reviewed_at FROM new_matches;
UPDATE public.person_claims cl SET person_id=CASE WHEN i.decision='pending' THEN cl.person_id ELSE i.person_id END,
 ...
 claim_json=cl.claim_json || jsonb_build_object('identityReviewRound11',i.review),
 scoring_version='cec-registration-identity-review-v12',scoring_reasons=coalesce(cl.scoring_reasons,'[]'::jsonb) || jsonb_build_array(jsonb_build_object('reason',i.review->>'reason','reasonCode',i.review->>'reasonCode','reviewedAt',i.review->>'reviewedAt')),updated_at=now()
...
DO $verify$ BEGIN
 IF (SELECT count(*) FROM new_candidates n JOIN published.candidates c ON c.candidate_id=n.id AND c.person_id=n.person_id AND c.race_id=n.race_id AND c.candidacy_status='registered' AND c.registration_status='registered' AND c.election_result='pending' AND c.candidate_no IS NULL)<>27
 THEN RAISE EXCEPTION 'Published candidate parity failed'; END IF;
 IF (SELECT count(*) FROM new_events n CROSS JOIN LATERAL published.candidate_lifecycle_for(n.candidate_id) ev WHERE ev.event_type='registration_filed' AND ev.occurred_on IS NOT DISTINCT FROM n.occurred_on AND ev.source_url=n.source_url AND ev.candidate_no IS NULL)<>27
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>1121
 THEN RAISE EXCEPTION 'Remaining pending count failed'; END IF;
END; $verify$;
UPDATE public.person_claims cl SET claim_json=cl.claim_json || jsonb_build_object('identityReviewRound11Audit',a.review),updated_at=now() FROM audit_input a WHERE cl.id=a.claim_id;
DO $audit_verify$ BEGIN
 IF (SELECT count(*) FROM audit_input a JOIN public.person_claims c ON c.id=a.claim_id WHERE c.claim_json->'identityReviewRound11Audit'=a.review AND NOT c.is_public AND c.visibility='review_only' AND c.claim_json->'registrationEvidence'=a.expected->'claim_json'->'registrationEvidence')<>1148 THEN RAISE EXCEPTION 'Comprehensive audit coverage or private boundary failed';END IF;
...
END; $audit_verify$;
SELECT json_build_object('mode',${quote(apply?'apply-local':'dry-run-rollback')},'release_id',(SELECT release_id FROM promoted),'reviewed',1148,'resolved',27,'reused_people',27,'new_people',0,'remaining_pending',1121,'newly_public_races',(SELECT count(*) FROM race_before WHERE NOT is_public),'before',json_build_object('claims',(SELECT json_agg(to_jsonb(cl)) FROM claims_before cl),'races',(SELECT json_agg(to_jsonb(r)) FROM race_before r WHERE NOT is_public)));
