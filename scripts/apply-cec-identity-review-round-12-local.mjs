...
assert(args.every(x=>x==='--apply-local'), 'Usage: node scripts/apply-cec-identity-review-round-12-local.mjs [--apply-local]');
const apply=args.includes('--apply-local');
const base=path.resolve('tmp/cec-registration-identity-review-12');
const archive=path.resolve('tmp/cec-registration-final');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,''));
const sha=x=>createHash('sha256').update(x).digest('hex');
const quote=x=>"'"+x.replaceAll("'","''")+"'";
const json=x=>quote(JSON.stringify(x))+'::jsonb';
const ledger=read('docs/cec-registration-identity-review-2026-09-05-round-12.json');
...
const normalizeGender=x=>x==='male'||x==='男'?'male':x==='female'||x==='女'?'female':null;
const nameKey=s=>s.normalize('NFKC').replace(/[\s．・‧•･.()（）]/g,'').toLowerCase();
const proofSources=evidence.sources.map(({canonical_person_id,match_status,match_method,...expected})=>({id:expected.id,canonical_person_id,expected}));
for(const r of accepted){
 assert.equal(r.decision,'use_existing');
 const p=r.identityComparison;assert(p.isPublic&&!p.conflicts.length&&!p.current.length);assert.equal(p.personId,r.personId);
 const src=evidence.sources.filter(s=>r.evidenceSourceIds.includes(s.id));
 assert(src.length);assert(src.every(s=>s.canonical_person_id===r.personId&&s.source_type==='official_officeholder'));
 assert(src.every(s=>nameKey(s.raw_name)===nameKey(r.name)||nameKey(s.source_payload?.originalLabel??'')===nameKey(r.name)));
 assert.deepEqual([...new Set(live.matched_sources.filter(s=>s.source_type==='official_officeholder'&&(nameKey(s.raw_name)===nameKey(r.name)||nameKey(s.source_payload?.originalLabel??'')===nameKey(r.name))).map(s=>s.canonical_person_id))],[r.personId]);
 const sex=normalizeGender(r.registrationGender);assert(p.genders.length<=1);if(sex)assert(p.genders.every(g=>g===sex));
 assert(r.externalEvidence.statement&&r.evidenceUrls.includes(r.externalEvidence.url));
 assert(['explicit_campaign_officeholder_bridge','official_original_full_name_same_district'].includes(r.reasonCode));
 if(r.reasonCode==='official_original_full_name_same_district'){
  assert.equal(r.name,'林庭秝 AliWalis');assert.equal(r.district,'南投縣第8選舉區');
  assert(src.some(s=>s.source_payload?.districtLabel==='第八選區'&&s.source_url.startsWith('https://www.ntcc.gov.tw/tw/rep/p02.aspx?district=8&period=20')));
 }
 for(const h of evidence.candidates.filter(c=>r.evidenceCandidateIds.includes(c.id))){assert.equal(h.canonical_person_id,r.personId);assert(h.is_public&&h.year<2026);}
 for(const id of r.evidenceClaimIds){const c=evidence.claims.find(c=>c.id===id);assert(c&&c.canonical_person_id===r.personId&&c.review_status==='verified');}
}
...
const sql=`BEGIN;
SET LOCAL application_name='cec-identity-review-round-12-20260905';
...
DO $check$ BEGIN
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>1121 THEN RAISE EXCEPTION 'Pending baseline changed';END IF;
 IF EXISTS(SELECT 1 FROM jsonb_to_recordset(${json(proofSources)}) x(id uuid,canonical_person_id uuid,expected jsonb) LEFT JOIN public.source_people s ON s.id=x.id WHERE s.id IS NULL OR to_jsonb(s) IS DISTINCT FROM x.expected OR NOT EXISTS(SELECT 1 FROM public.person_identity_matches m JOIN public.person_canonical_map pm ON pm.person_id=m.person_id WHERE m.source_person_id=x.id AND m.match_status IN ('auto_matched','confirmed') AND pm.canonical_person_id=x.canonical_person_id)) THEN RAISE EXCEPTION 'Official officeholder identity evidence changed';END IF;
...
SELECT id,source_person_id,person_id,'auto_matched',100,'cec_registration_identity_review_v10',match_reason,evidence_json,reviewed_by,reviewed_at,reviewed_at FROM new_matches;
UPDATE public.person_claims cl SET person_id=CASE WHEN i.decision='pending' THEN cl.person_id ELSE i.person_id END,
 ...
 claim_json=cl.claim_json || jsonb_build_object('identityReviewRound12',i.review),
 scoring_version='cec-registration-identity-review-v13',scoring_reasons=coalesce(cl.scoring_reasons,'[]'::jsonb) || jsonb_build_array(jsonb_build_object('reason',i.review->>'reason','reasonCode',i.review->>'reasonCode','reviewedAt',i.review->>'reviewedAt')),updated_at=now()
...
DO $verify$ BEGIN
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>1104
 THEN RAISE EXCEPTION 'Remaining pending count failed'; END IF;
END; $verify$;
UPDATE public.person_claims cl SET claim_json=cl.claim_json || jsonb_build_object('identityReviewRound12Audit',a.review),updated_at=now() FROM audit_input a WHERE cl.id=a.claim_id;
DO $audit_verify$ BEGIN
 IF (SELECT count(*) FROM audit_input a JOIN public.person_claims c ON c.id=a.claim_id WHERE c.claim_json->'identityReviewRound12Audit'=a.review AND NOT c.is_public AND c.visibility='review_only' AND c.claim_json->'registrationEvidence'=a.expected->'claim_json'->'registrationEvidence')<>17 THEN RAISE EXCEPTION 'Comprehensive audit coverage or private boundary failed';END IF;
...
END; $audit_verify$;
SELECT json_build_object('mode',${quote(apply?'apply-local':'dry-run-rollback')},'release_id',(SELECT release_id FROM promoted),'reviewed',17,'resolved',17,'reused_people',17,'new_people',0,'remaining_pending',1104,'newly_public_races',(SELECT count(*) FROM race_before WHERE NOT is_public),'before',json_build_object('claims',(SELECT json_agg(to_jsonb(cl)) FROM claims_before cl),'races',(SELECT json_agg(to_jsonb(r)) FROM race_before r WHERE NOT is_public)));
