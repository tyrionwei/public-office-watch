...
assert(args.every(x=>x==='--apply-local'), 'Usage: node scripts/apply-cec-identity-review-round-13-local.mjs [--apply-local]');
const apply=args.includes('--apply-local');
const base=path.resolve('tmp/cec-registration-identity-review-13');
const archive=path.resolve('tmp/cec-registration-final');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,''));
const sha=x=>createHash('sha256').update(x).digest('hex');
const quote=x=>"'"+x.replaceAll("'","''")+"'";
const json=x=>quote(JSON.stringify(x))+'::jsonb';
const ledger=read('docs/cec-registration-identity-review-2026-09-05-round-13.json');
...
const year=s=>{const m=String(s??'').match(/^(\d{3,4})(?:[-/]|$)/);return m?Number(m[1])+(m[1].length===3?1911:0):null;};
for(const r of accepted){
 assert.equal(r.decision,'create_new');assert.equal(r.reasonCode,'distinct_birth_year_from_all_existing_namesakes');
 const registration=targets.get(r.candidateExternalId).claim.claim_json.registrationEvidence;
 assert.equal(year(registration.birth_date_raw),r.registrationBirthYear);
 assert.equal(baseline.filter(c=>nameKey(c.claim_json.registrationEvidence.name)===nameKey(r.name)).length,1);
 const known=[...new Set([...live.people.filter(p=>nameKey(p.name)===nameKey(r.name)).map(p=>p.canonical_person_id),...live.matched_sources.filter(s=>s.source_type==='official_officeholder'&&nameKey(s.source_payload?.originalLabel??s.raw_name)===nameKey(r.name)).map(s=>s.canonical_person_id)])].sort();
 assert.deepEqual(known,[...r.existingPersonIds].sort());assert(known.length);
 assert.equal(r.identityExclusions.length,known.length);
 for(const p of r.identityExclusions){
  assert(known.includes(p.personId)&&p.birthYears.length===1&&p.birthYears[0]!==r.registrationBirthYear&&p.genders.length<=1&&!p.exactLocal.length);
  const cl=evidence.claims.filter(c=>r.evidenceClaimIds.includes(c.id)&&c.canonical_person_id===p.personId);
  assert(cl.every(c=>c.review_status==='verified'));
  const years=[...new Set(cl.map(c=>['birth_date','birth_year'].includes(c.claim_type)?year(c.claim_value):year(c.claim_json?.registrationEvidence?.birth_date_raw)).filter(Boolean))];
  assert.deepEqual(years,p.birthYears);
  for(const c of cl){const source=c.claim_json?.registrationEvidence?.source;if(source){assert.equal(new URL(source.url).hostname,'web.cec.gov.tw');const file=path.resolve(archive,source.file);assert(file.startsWith(archive+path.sep));assert.equal(sha(fs.readFileSync(file)),source.sha256);checkedFiles.add(file);}}
 }
}
...
const sql=`BEGIN;
SET LOCAL application_name='cec-identity-review-round-13-20260905';
...
DO $check$ BEGIN
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>1104 THEN RAISE EXCEPTION 'Pending baseline changed';END IF;
...
SELECT id,source_person_id,person_id,'auto_matched',100,'cec_registration_identity_review_v11',match_reason,evidence_json,reviewed_by,reviewed_at,reviewed_at FROM new_matches;
UPDATE public.person_claims cl SET person_id=CASE WHEN i.decision='pending' THEN cl.person_id ELSE i.person_id END,
 ...
 claim_json=cl.claim_json || jsonb_build_object('identityReviewRound13',i.review),
 scoring_version='cec-registration-identity-review-v14',scoring_reasons=coalesce(cl.scoring_reasons,'[]'::jsonb) || jsonb_build_array(jsonb_build_object('reason',i.review->>'reason','reasonCode',i.review->>'reasonCode','reviewedAt',i.review->>'reviewedAt')),updated_at=now()
...
DO $verify$ BEGIN
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>1087
 THEN RAISE EXCEPTION 'Remaining pending count failed'; END IF;
END; $verify$;
UPDATE public.person_claims cl SET claim_json=cl.claim_json || jsonb_build_object('identityReviewRound13Audit',a.review),updated_at=now() FROM audit_input a WHERE cl.id=a.claim_id;
DO $audit_verify$ BEGIN
 IF (SELECT count(*) FROM audit_input a JOIN public.person_claims c ON c.id=a.claim_id WHERE c.claim_json->'identityReviewRound13Audit'=a.review AND NOT c.is_public AND c.visibility='review_only' AND c.claim_json->'registrationEvidence'=a.expected->'claim_json'->'registrationEvidence')<>17 THEN RAISE EXCEPTION 'Comprehensive audit coverage or private boundary failed';END IF;
...
END; $audit_verify$;
SELECT json_build_object('mode',${quote(apply?'apply-local':'dry-run-rollback')},'release_id',(SELECT release_id FROM promoted),'reviewed',17,'resolved',17,'reused_people',0,'new_people',17,'remaining_pending',1087,'newly_public_races',(SELECT count(*) FROM race_before WHERE NOT is_public),'before',json_build_object('claims',(SELECT json_agg(to_jsonb(cl)) FROM claims_before cl),'races',(SELECT json_agg(to_jsonb(r)) FROM race_before r WHERE NOT is_public)));
