...
assert(args.every(x=>x==='--apply-local'), 'Usage: node scripts/apply-cec-identity-review-round-17-local.mjs [--apply-local]');
const apply=args.includes('--apply-local');
const base=path.resolve('tmp/cec-registration-identity-review-17');
const archive=path.resolve('tmp/cec-registration-final');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,''));
const sha=x=>createHash('sha256').update(x).digest('hex');
const quote=x=>"'"+x.replaceAll("'","''")+"'";
const json=x=>quote(JSON.stringify(x))+'::jsonb';
const ledger=read('docs/cec-registration-identity-review-2026-09-05-round-17.json');
...
assert.equal(audit.length,948);assert.equal(new Set(audit.map(x=>x.claimId)).size,948);
...
assert.equal(review.environment,'full-local');assert.equal(review.records.length,2);
...
assert.equal(new Set(accepted.map(r=>r.personId)).size,2);
assert.equal(new Set(review.records.map(r=>r.claimId)).size,2);
...
for(const r of accepted){
 ...
 const c=read(path.join(base,'classification.json')).find(c=>c.claimId===r.claimId);
 const p=c.comparisons.find(p=>p.personId===r.personId);
 assert(p&&p.isPublic&&!p.conflicts.length&&!p.current.length);
 assert.deepEqual(r.identityComparison,p);
 assert((r.evidenceCandidateIds.length||r.externalEvidence.officialRoster)&&r.externalEvidence.url&&r.evidenceUrls.includes(r.externalEvidence.url));
 if(r.externalEvidence.officialRoster){assert.equal(r.name,'許采蓁');assert.equal(r.registrationBirth,r.externalEvidence.verifiedBirthDate);assert(live.people.some(x=>x.id===r.personId&&x.source_url===r.externalEvidence.rosterUrl));assert.equal(sha(fs.readFileSync(r.externalEvidence.archiveFile)),r.externalEvidence.archiveSha256);}else assert(p.countyHistory.some(h=>h.title.startsWith(r.externalEvidence.historyPrefix)));
 if(r.externalEvidence.correctedName){assert.equal(r.name,'林秀美 Rngos(cid:504)Ofo');assert.equal(r.externalEvidence.correctedName,'林秀美 Rngos·Ofo');assert(p.exactLocal.length);}
 if(r.name==='吳亮慶')assert(p.birthDates.includes('1954-04-08'));
 for(const id of r.evidenceClaimIds){const cl=evidence.claims.find(cl=>cl.id===id);assert(cl&&cl.canonical_person_id===r.personId&&cl.review_status==='verified');}
 for(const id of r.evidenceCandidateIds){const h=evidence.candidates.find(h=>h.id===id);assert(h&&h.canonical_person_id===r.personId&&h.is_public&&h.year<2026);}
}
...
const sql=`BEGIN;
SET LOCAL application_name='cec-identity-review-round-17-20260905';
...
DO $check$ BEGIN
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>948 THEN RAISE EXCEPTION 'Pending baseline changed';END IF;
 IF (SELECT count(*) FROM claims_before)<>2 OR (SELECT count(*) FROM sources_before)<>2 THEN RAISE EXCEPTION 'Incomplete pending review inputs'; END IF;
...
SELECT id,source_person_id,person_id,'auto_matched',100,'cec_registration_identity_review_v17',match_reason,evidence_json,reviewed_by,reviewed_at,reviewed_at FROM new_matches;
UPDATE public.person_claims cl SET person_id=CASE WHEN i.decision='pending' THEN cl.person_id ELSE i.person_id END,
 ...
 claim_json=cl.claim_json || jsonb_build_object('identityReviewRound17',i.review),
 scoring_version='cec-registration-identity-review-v18',scoring_reasons=coalesce(cl.scoring_reasons,'[]'::jsonb) || jsonb_build_array(jsonb_build_object('reason',i.review->>'reason','reasonCode',i.review->>'reasonCode','reviewedAt',i.review->>'reviewedAt')),updated_at=now()
...
DO $verify$ BEGIN
 IF (SELECT count(*) FROM new_candidates n JOIN published.candidates c ON c.candidate_id=n.id AND c.person_id=n.person_id AND c.race_id=n.race_id AND c.candidacy_status='registered' AND c.registration_status='registered' AND c.election_result='pending' AND c.candidate_no IS NULL)<>2
 THEN RAISE EXCEPTION 'Published candidate parity failed'; END IF;
 IF (SELECT count(*) FROM new_events n CROSS JOIN LATERAL published.candidate_lifecycle_for(n.candidate_id) ev WHERE ev.event_type='registration_filed' AND ev.occurred_on IS NOT DISTINCT FROM n.occurred_on AND ev.source_url=n.source_url AND ev.candidate_no IS NULL)<>2
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>946
 THEN RAISE EXCEPTION 'Remaining pending count failed'; END IF;
END; $verify$;
UPDATE public.person_claims cl SET claim_json=cl.claim_json || jsonb_build_object('identityReviewRound17Audit',a.review),updated_at=now() FROM audit_input a WHERE cl.id=a.claim_id;
DO $audit_verify$ BEGIN
 IF (SELECT count(*) FROM audit_input a JOIN public.person_claims c ON c.id=a.claim_id WHERE c.claim_json->'identityReviewRound17Audit'=a.review AND NOT c.is_public AND c.visibility='review_only' AND c.claim_json->'registrationEvidence'=a.expected->'claim_json'->'registrationEvidence')<>948 THEN RAISE EXCEPTION 'Comprehensive audit coverage or private boundary failed';END IF;
