...
assert(args.every(x=>x==='--apply-local'), 'Usage: node scripts/apply-cec-identity-review-round-18-local.mjs [--apply-local]');
const apply=args.includes('--apply-local');
const base=path.resolve('tmp/cec-registration-identity-review-18');
const archive=path.resolve('tmp/cec-registration-final');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,''));
const sha=x=>createHash('sha256').update(x).digest('hex');
const quote=x=>"'"+x.replaceAll("'","''")+"'";
const json=x=>quote(JSON.stringify(x))+'::jsonb';
const ledger=read('docs/cec-registration-identity-review-2026-09-05-round-18.json');
...
assert.equal(audit.length,9);assert.equal(new Set(audit.map(x=>x.claimId)).size,9);
...
assert.equal(review.environment,'full-local');assert.equal(review.records.length,9);
...
assert.equal(new Set(accepted.map(r=>r.personId)).size,9);
assert.equal(new Set(review.records.map(r=>r.claimId)).size,9);
...
const sql=`BEGIN;
SET LOCAL application_name='cec-identity-review-round-18-20260905';
...
DO $check$ BEGIN
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>946 THEN RAISE EXCEPTION 'Pending baseline changed';END IF;
 IF (SELECT count(*) FROM claims_before)<>9 OR (SELECT count(*) FROM sources_before)<>9 THEN RAISE EXCEPTION 'Incomplete pending review inputs'; END IF;
...
SELECT id,source_person_id,person_id,'auto_matched',100,'cec_registration_identity_review_v18',match_reason,evidence_json,reviewed_by,reviewed_at,reviewed_at FROM new_matches;
UPDATE public.person_claims cl SET person_id=CASE WHEN i.decision='pending' THEN cl.person_id ELSE i.person_id END,
 ...
 claim_json=cl.claim_json || jsonb_build_object('identityReviewRound18',i.review),
...
DO $verify$ BEGIN
 IF (SELECT count(*) FROM new_candidates n JOIN published.candidates c ON c.candidate_id=n.id AND c.person_id=n.person_id AND c.race_id=n.race_id AND c.candidacy_status='registered' AND c.registration_status='registered' AND c.election_result='pending' AND c.candidate_no IS NULL)<>9
 THEN RAISE EXCEPTION 'Published candidate parity failed'; END IF;
 IF (SELECT count(*) FROM new_events n CROSS JOIN LATERAL published.candidate_lifecycle_for(n.candidate_id) ev WHERE ev.event_type='registration_filed' AND ev.occurred_on IS NOT DISTINCT FROM n.occurred_on AND ev.source_url=n.source_url AND ev.candidate_no IS NULL)<>9
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>937
 THEN RAISE EXCEPTION 'Remaining pending count failed'; END IF;
END; $verify$;
UPDATE public.person_claims cl SET claim_json=cl.claim_json || jsonb_build_object('identityReviewRound18Audit',a.review),updated_at=now() FROM audit_input a WHERE cl.id=a.claim_id;
DO $audit_verify$ BEGIN
 IF (SELECT count(*) FROM audit_input a JOIN public.person_claims c ON c.id=a.claim_id WHERE c.claim_json->'identityReviewRound18Audit'=a.review AND NOT c.is_public AND c.visibility='review_only' AND c.claim_json->'registrationEvidence'=a.expected->'claim_json'->'registrationEvidence')<>9 THEN RAISE EXCEPTION 'Comprehensive audit coverage or private boundary failed';END IF;
...
END; $audit_verify$;
SELECT json_build_object('mode',${quote(apply?'apply-local':'dry-run-rollback')},'release_id',(SELECT release_id FROM promoted),'reviewed',9,'resolved',9,'reused_people',3,'new_people',6,'remaining_pending',937,'newly_public_races',(SELECT count(*) FROM race_before WHERE NOT is_public),'before',json_build_object('claims',(SELECT json_agg(to_jsonb(cl)) FROM claims_before cl),'races',(SELECT json_agg(to_jsonb(r)) FROM race_before r WHERE NOT is_public)));
