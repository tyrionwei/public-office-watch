...
assert(args.every(x=>x==='--apply-local'), 'Usage: node scripts/apply-cec-identity-review-round-5-local.mjs [--apply-local]');
const apply=args.includes('--apply-local');
const base=path.resolve('tmp/cec-registration-identity-review-5');
const archive=path.resolve('tmp/cec-registration-final');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,''));
const sha=x=>createHash('sha256').update(x).digest('hex');
const quote=x=>"'"+x.replaceAll("'","''")+"'";
const json=x=>quote(JSON.stringify(x))+'::jsonb';
const ledger=read('docs/cec-registration-identity-review-2026-09-05-round-5.json');
...
assert.equal(externalByName.size,20);
...
assert.equal(review.environment,'full-local');assert.equal(review.records.length,26);
...
assert.equal(new Set(accepted.map(r=>r.personId)).size,26);
assert.equal(new Set(review.records.map(r=>r.claimId)).size,26);
...
for(const r of accepted.filter(x=>x.decision==='use_existing')){
 ...
 assert(evidence.people.some(x=>x.canonical_person_id===r.personId&&normalizeGender(x.gender)===normalizeGender(r.registrationGender)),r.name+' normalized gender evidence missing');
 ...
 if(r.reasonCode==='reported_current_role_transition_after_gender_normalization'){
  const external=externalByName.get(r.name);
  assert(external&&external.url===r.externalEvidence?.url&&external.statement===r.externalEvidence?.statement&&r.evidenceUrls.includes(external.url));
  assert.equal(new URL(external.url).protocol,'https:');
 }else if(r.reasonCode==='masked_birth_year_and_official_history_after_gender_normalization'){
  ...
  assert(String(historical.title).includes(r.county));
  assert.equal(normalizeParty(historical.party),normalizeParty(r.registrationParty));
 }else if(r.reasonCode==='exact_2018_race_gender_party_unique_person'){
  assert.equal(historical.year,2018);assert.equal(historical.race_type,'village_chief');assert(String(historical.title).includes(r.district));
  assert.equal(normalizeParty(historical.party),normalizeParty(r.registrationParty));
 }else assert.fail('Unexpected review reason: '+r.reasonCode);
}
const sql=`BEGIN;
SET LOCAL application_name='cec-identity-review-round-5-20260905';
...
DO $check$ BEGIN
 IF (SELECT count(*) FROM claims_before)<>26 OR (SELECT count(*) FROM sources_before)<>26 THEN RAISE EXCEPTION 'Incomplete pending review inputs'; END IF;
...
SELECT id,source_person_id,person_id,'auto_matched',100,'cec_registration_identity_review_v3',match_reason,evidence_json,reviewed_by,reviewed_at,reviewed_at FROM new_matches;
UPDATE public.person_claims cl SET person_id=CASE WHEN i.decision='pending' THEN cl.person_id ELSE i.person_id END,
 ...
 claim_json=cl.claim_json || jsonb_build_object('identityReviewRound5',i.review),
 scoring_version='cec-registration-identity-review-v6',scoring_reasons=coalesce(cl.scoring_reasons,'[]'::jsonb) || jsonb_build_array(jsonb_build_object('reason',i.review->>'reason','reasonCode',i.review->>'reasonCode','reviewedAt',i.review->>'reviewedAt')),updated_at=now()
...
DO $verify$ BEGIN
 IF (SELECT count(*) FROM new_candidates n JOIN published.candidates c ON c.candidate_id=n.id AND c.person_id=n.person_id AND c.race_id=n.race_id AND c.candidacy_status='registered' AND c.registration_status='registered' AND c.election_result='pending' AND c.candidate_no IS NULL)<>26
 THEN RAISE EXCEPTION 'Published candidate parity failed'; END IF;
 IF (SELECT count(*) FROM new_events n CROSS JOIN LATERAL published.candidate_lifecycle_for(n.candidate_id) ev WHERE ev.event_type='registration_filed' AND ev.occurred_on IS NOT DISTINCT FROM n.occurred_on AND ev.source_url=n.source_url AND ev.candidate_no IS NULL)<>26
