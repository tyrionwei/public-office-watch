...
assert(args.every(x=>x==='--apply-local'), 'Usage: node scripts/apply-cec-identity-review-round-6-local.mjs [--apply-local]');
const apply=args.includes('--apply-local');
const base=path.resolve('tmp/cec-registration-identity-review-6');
const archive=path.resolve('tmp/cec-registration-final');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,''));
const sha=x=>createHash('sha256').update(x).digest('hex');
const quote=x=>"'"+x.replaceAll("'","''")+"'";
const json=x=>quote(JSON.stringify(x))+'::jsonb';
const ledger=read('docs/cec-registration-identity-review-2026-09-05-round-6.json');
...
const externalEvidence=read(path.join(base,'external-evidence.json'));
const externalByName=new Map(externalEvidence.map(x=>[x.name,x]));
assert.equal(externalByName.size,6);
...
assert.equal(review.environment,'full-local');assert.equal(review.records.length,6);
...
assert.equal(new Set(accepted.map(r=>r.personId)).size,6);
assert.equal(new Set(review.records.map(r=>r.claimId)).size,6);
...
const normalizeGender=x=>x==='male'||x?.codePointAt(0)===0x7537?'male':x==='female'||x?.codePointAt(0)===0x5973?'female':null;
const normalizeParty=x=>x?.startsWith(String.fromCodePoint(0x7121))?String.fromCodePoint(0x7121):x??null;
for(const r of accepted){
 const external=externalByName.get(r.name);
 assert(external&&external.url===r.externalEvidence?.url&&external.statement===r.externalEvidence?.statement,r.name+' external evidence mismatch');
 assert.equal(new URL(external.url).protocol,'https:');
 assert(r.evidenceUrls.includes(external.url));
 for(const url of external.supportingUrls??[])assert(r.evidenceUrls.includes(url),r.name+' supporting URL missing');
 if(r.decision==='create_new'){
  assert.equal(r.name,'王志偉');
  assert.equal(r.registrationBirth,'049/**/**');
  assert(!r.existingPersonIds.includes(r.personId));
  assert.equal(r.evidenceClaimIds.length,0);
  assert.equal(r.evidenceCandidateIds.length,0);
  assert(external.statement.includes('66'));
  continue;
 }
 assert(r.existingPersonIds.includes(r.personId),r.name+' selected person was not an existing same-name option');
 for(const id of r.evidenceClaimIds)assert.equal(evidence.claims.find(x=>x.id===id)?.canonical_person_id,r.personId);
 for(const id of r.evidenceCandidateIds)assert.equal(evidence.candidates.find(x=>x.id===id)?.canonical_person_id,r.personId);
 const historical=evidence.candidates.find(x=>r.evidenceCandidateIds.includes(x.id));
 assert(historical&&historical.canonical_person_id===r.personId);
 if(r.registrationGender){
  assert(evidence.people.some(x=>x.canonical_person_id===r.personId&&normalizeGender(x.gender)===normalizeGender(r.registrationGender))||evidence.claims.some(x=>x.canonical_person_id===r.personId&&x.claim_type==='gender'&&x.review_status==='verified'&&normalizeGender(x.claim_value)===normalizeGender(r.registrationGender)),r.name+' normalized gender evidence missing');
 }
 if(r.reasonCode==='reported_election_history_selects_same_name_person'){
  assert.equal(r.name,'吳炳輝');
  assert.equal(historical.year,2024);
  assert(String(historical.title).includes('雲林縣第1選舉區'));
  assert(r.evidenceClaimIds.some(id=>{const c=evidence.claims.find(x=>x.id===id);return c?.claim_type==='birth_date'&&c.review_status==='verified'&&String(c.claim_value).startsWith('1967-');}));
 }else if(r.reasonCode==='same_gender_party_and_2024_history_select_same_name_person'){
  assert.equal(r.name,'洪麗華');
  assert.equal(historical.year,2024);
  assert.equal(normalizeParty(historical.party),normalizeParty(r.registrationParty));
  assert(String(historical.title).includes('臺中市第2選舉區'));
 }else if(r.reasonCode==='masked_birth_year_and_exact_2022_local_history_select_person'){
  assert.equal(r.name,'劉燦輝');
  const year=String(Number(r.registrationBirth.slice(0,3))+1911);
  assert.equal(year,'1963');
  assert(r.evidenceClaimIds.some(id=>{const c=evidence.claims.find(x=>x.id===id);return c?.claim_type==='birth_date'&&c.review_status==='verified'&&String(c.claim_value).startsWith(year+'-');}));
  assert.equal(historical.year,2022);
  assert.equal(historical.race_type,'township_mayor');
  assert(String(historical.title).includes('員山鄉'));
 }else if(r.reasonCode==='reported_role_and_2022_candidacy_link_unique_person'){
  assert.equal(r.name,'許智富');
  assert.equal(r.existingPersonIds.length,1);
  assert.equal(historical.year,2022);
  assert.equal(historical.race_type,'county_councilor');
  assert(String(historical.title).includes('澎湖縣'));
 }else if(r.reasonCode==='reported_former_party_role_and_2022_candidacy_link_unique_person'){
  assert.equal(r.name,'何志勇');
  assert.equal(r.existingPersonIds.length,1);
  assert.equal(historical.year,2022);
  assert.equal(historical.race_type,'city_councilor');
  assert(String(historical.title).includes('新竹市'));
 }else assert.fail('Unexpected review reason: '+r.reasonCode);
}
const sql=`BEGIN;
SET LOCAL application_name='cec-identity-review-round-6-20260905';
...
DO $check$ BEGIN
 IF (SELECT count(*) FROM claims_before)<>6 OR (SELECT count(*) FROM sources_before)<>6 THEN RAISE EXCEPTION 'Incomplete pending review inputs'; END IF;
...
SELECT id,source_person_id,person_id,'auto_matched',100,'cec_registration_identity_review_v4',match_reason,evidence_json,reviewed_by,reviewed_at,reviewed_at FROM new_matches;
UPDATE public.person_claims cl SET person_id=CASE WHEN i.decision='pending' THEN cl.person_id ELSE i.person_id END,
 ...
 claim_json=cl.claim_json || jsonb_build_object('identityReviewRound6',i.review),
 scoring_version='cec-registration-identity-review-v7',scoring_reasons=coalesce(cl.scoring_reasons,'[]'::jsonb) || jsonb_build_array(jsonb_build_object('reason',i.review->>'reason','reasonCode',i.review->>'reasonCode','reviewedAt',i.review->>'reviewedAt')),updated_at=now()
...
DO $verify$ BEGIN
 IF (SELECT count(*) FROM new_candidates n JOIN published.candidates c ON c.candidate_id=n.id AND c.person_id=n.person_id AND c.race_id=n.race_id AND c.candidacy_status='registered' AND c.registration_status='registered' AND c.election_result='pending' AND c.candidate_no IS NULL)<>6
 THEN RAISE EXCEPTION 'Published candidate parity failed'; END IF;
 IF (SELECT count(*) FROM new_events n CROSS JOIN LATERAL published.candidate_lifecycle_for(n.candidate_id) ev WHERE ev.event_type='registration_filed' AND ev.occurred_on IS NOT DISTINCT FROM n.occurred_on AND ev.source_url=n.source_url AND ev.candidate_no IS NULL)<>6
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>1192
 THEN RAISE EXCEPTION 'Remaining pending count failed'; END IF;
END; $verify$;
SELECT json_build_object('mode',${quote(apply?'apply-local':'dry-run-rollback')},'release_id',(SELECT release_id FROM promoted),'reviewed',6,'resolved',6,'reused_people',5,'new_people',1,'remaining_pending',1192,'newly_public_races',(SELECT count(*) FROM race_before WHERE NOT is_public),'before',json_build_object('claims',(SELECT json_agg(to_jsonb(cl)) FROM claims_before cl),'races',(SELECT json_agg(to_jsonb(r)) FROM race_before r WHERE NOT is_public)));
