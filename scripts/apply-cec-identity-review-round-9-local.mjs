...
assert(args.every(x=>x==='--apply-local'), 'Usage: node scripts/apply-cec-identity-review-round-9-local.mjs [--apply-local]');
const apply=args.includes('--apply-local');
const base=path.resolve('tmp/cec-registration-identity-review-9');
const archive=path.resolve('tmp/cec-registration-final');
const read=p=>JSON.parse(fs.readFileSync(p,'utf8').replace(/^\uFEFF/,''));
const sha=x=>createHash('sha256').update(x).digest('hex');
const quote=x=>"'"+x.replaceAll("'","''")+"'";
const json=x=>quote(JSON.stringify(x))+'::jsonb';
const ledger=read('docs/cec-registration-identity-review-2026-09-05-round-9.json');
...
assert.equal(externalByName.size,18);
...
assert.equal(review.environment,'full-local');assert.equal(review.records.length,18);
...
assert.equal(new Set(accepted.map(r=>r.personId)).size,18);
assert.equal(new Set(review.records.map(r=>r.claimId)).size,18);
...
for(const r of accepted){
 ...
 assert.equal(r.existingPersonIds[0],r.personId);
 assert.equal(r.reasonCode,'reported_current_local_role_and_2022_elected_record_unique_person');
 ...
 const genderClaim=evidence.claims.find(x=>r.evidenceClaimIds.includes(x.id)&&x.claim_type==='gender'&&x.review_status==='verified');
 assert(genderClaim&&normalizeGender(genderClaim.claim_value)===normalizeGender(r.registrationGender),r.name+' normalized gender evidence missing');
 ...
 assert.equal(historical.is_elected,true);
 assert(String(historical.title).includes(r.district.replace(r.county,'')),r.name+' local history does not match current district');
...
const sql=`BEGIN;
SET LOCAL application_name='cec-identity-review-round-9-20260905';
...
DO $check$ BEGIN
 IF (SELECT count(*) FROM claims_before)<>18 OR (SELECT count(*) FROM sources_before)<>18 THEN RAISE EXCEPTION 'Incomplete pending review inputs'; END IF;
 ...
 IF EXISTS(SELECT 1 FROM input i WHERE
 (SELECT count(DISTINCT pm.canonical_person_id) FROM public.people p JOIN public.person_canonical_map pm ON pm.person_id=p.id WHERE p.name=i.review->>'name')<>1
 OR NOT EXISTS(SELECT 1 FROM public.people p JOIN public.person_canonical_map pm ON pm.person_id=p.id WHERE p.name=i.review->>'name' AND pm.canonical_person_id=i.person_id))
 THEN RAISE EXCEPTION 'Live same-name canonical identities changed'; END IF;
...
SELECT id,source_person_id,person_id,'auto_matched',100,'cec_registration_identity_review_v7',match_reason,evidence_json,reviewed_by,reviewed_at,reviewed_at FROM new_matches;
UPDATE public.person_claims cl SET person_id=CASE WHEN i.decision='pending' THEN cl.person_id ELSE i.person_id END,
 ...
 claim_json=cl.claim_json || jsonb_build_object('identityReviewRound9',i.review),
 scoring_version='cec-registration-identity-review-v10',scoring_reasons=coalesce(cl.scoring_reasons,'[]'::jsonb) || jsonb_build_array(jsonb_build_object('reason',i.review->>'reason','reasonCode',i.review->>'reasonCode','reviewedAt',i.review->>'reviewedAt')),updated_at=now()
...
DO $verify$ BEGIN
 IF (SELECT count(*) FROM new_candidates n JOIN published.candidates c ON c.candidate_id=n.id AND c.person_id=n.person_id AND c.race_id=n.race_id AND c.candidacy_status='registered' AND c.registration_status='registered' AND c.election_result='pending' AND c.candidate_no IS NULL)<>18
 THEN RAISE EXCEPTION 'Published candidate parity failed'; END IF;
 IF (SELECT count(*) FROM new_events n CROSS JOIN LATERAL published.candidate_lifecycle_for(n.candidate_id) ev WHERE ev.event_type='registration_filed' AND ev.occurred_on IS NOT DISTINCT FROM n.occurred_on AND ev.source_url=n.source_url AND ev.candidate_no IS NULL)<>18
 ...
 IF (SELECT count(*) FROM public.person_claims WHERE id IN (SELECT value::uuid FROM jsonb_array_elements_text(${json(manifest.map(x=>x.claimId))})) AND review_status='pending')<>1156
 THEN RAISE EXCEPTION 'Remaining pending count failed'; END IF;
END; $verify$;
SELECT json_build_object('mode',${quote(apply?'apply-local':'dry-run-rollback')},'release_id',(SELECT release_id FROM promoted),'reviewed',18,'resolved',18,'reused_people',18,'new_people',0,'remaining_pending',1156,'newly_public_races',(SELECT count(*) FROM race_before WHERE NOT is_public),'before',json_build_object('claims',(SELECT json_agg(to_jsonb(cl)) FROM claims_before cl),'races',(SELECT json_agg(to_jsonb(r)) FROM race_before r WHERE NOT is_public)));
