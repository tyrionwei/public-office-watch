...
const outputFiles = [
  '20260820152016_publish_production_profile_and_finance_delta.sql',
  '20260820163403_load_production_release_claims_1.sql',
  '20260820163410_load_production_release_claims_2.sql',
  '20260820163416_load_production_release_claims_3.sql',
  '20260820163422_load_production_release_claims_4.sql',
  '20260820163429_load_production_release_affiliations_1.sql',
  '20260820163438_load_production_release_affiliations_2.sql',
  '20260820163444_finalize_production_profile_and_finance_release.sql',
];
...
function columnDeclaration(group) {
  return columns[group].map(([column, type]) => `${column} ${type}`).join(', ');
}

function rowInserts(name, group, rows, delimiter) {
  const declaration = columnDeclaration(group);
  const inserts = [];
...
function stagingTable(name, group, rows, delimiter) {
  const inserts = rowInserts(name, group, rows, delimiter);
  return `CREATE UNLOGGED TABLE ${name} (${columnDeclaration(group)});

${inserts}`;
}

function stagingKeyTable(name, rows, declaration, delimiter) {
  return `CREATE UNLOGGED TABLE ${name} (${declaration});
...
function stagedTempTable(name) {
  const suffix = name.replace(/^_release_/u, '');
  return `CREATE TEMP TABLE ${name} ON COMMIT DROP AS
SELECT * FROM release_staging_20260820._production_release_${suffix}_20260820;`;
}

function tempTable(name) {
  return stagedTempTable(name);
}
...
function splitRows(rows, partCount) {
  const partSize = Math.ceil(rows.length / partCount);
  return Array.from(
    { length: partCount },
    (_, index) => rows.slice(index * partSize, (index + 1) * partSize),
  );
}

function loadMigration(name, group, rows, expectedCount, delimiter) {
  return `-- Generated by scripts/build-production-profile-finance-release.mjs
...
${rowInserts(name, group, rows, delimiter)}

DO $$
BEGIN
    IF (SELECT COUNT(*) FROM ${name}) <> ${expectedCount} THEN
        RAISE EXCEPTION 'Production release staging count drift for ${name}';
    END IF;
END
$$;
...
async function main() {
  ...
  const staging = {
    people: 'release_staging_20260820._production_release_people_20260820',
    candidates: 'release_staging_20260820._production_release_candidates_20260820',
    claims: 'release_staging_20260820._production_release_claims_20260820',
    affiliations: 'release_staging_20260820._production_release_affiliations_20260820',
    finance: 'release_staging_20260820._production_release_finance_20260820',
    peopleKeys: 'release_staging_20260820._production_release_people_keys_20260820',
    candidateKeys: 'release_staging_20260820._production_release_candidate_keys_20260820',
    raceKeys: 'release_staging_20260820._production_release_race_keys_20260820',
    partyKeys: 'release_staging_20260820._production_release_party_keys_20260820',
  };
  const stagingTables = Object.values(staging).join(',\n    ');
  const claimParts = splitRows(claims, 4);
  const affiliationParts = splitRows(affiliations, 2);
  let expectedClaimCount = 0;
  let expectedAffiliationCount = 0;

  const setupSql = `-- Generated by scripts/build-production-profile-finance-release.mjs
-- from final reviewed rows in the full local Supabase database.
BEGIN;
...
CREATE SCHEMA IF NOT EXISTS release_staging_20260820;
REVOKE ALL ON SCHEMA release_staging_20260820 FROM PUBLIC, anon, authenticated;

DROP TABLE IF EXISTS
    ${stagingTables};

${stagingTable(staging.people, 'people', people, delimiter)}

${stagingTable(staging.candidates, 'candidates', candidates, delimiter)}

${stagingTable(staging.claims, 'claims', [], delimiter)}

${stagingTable(staging.affiliations, 'affiliations', [], delimiter)}

${stagingTable(staging.finance, 'finance', finance, delimiter)}

${stagingKeyTable(staging.peopleKeys, peopleKeys, 'local_id uuid, external_id text', delimiter)}

${stagingKeyTable(staging.candidateKeys, candidateKeys, 'local_id uuid, external_id text', delimiter)}

${stagingKeyTable(staging.raceKeys, raceKeys, 'local_id uuid, external_id text', delimiter)}

${stagingKeyTable(staging.partyKeys, partyKeys, 'local_id uuid, external_id text', delimiter)}
...
DO $$
BEGIN
    IF (SELECT COUNT(*) FROM ${staging.people}) <> ${people.length}
       OR (SELECT COUNT(*) FROM ${staging.candidates}) <> ${candidates.length}
       OR (SELECT COUNT(*) FROM ${staging.claims}) <> 0
       OR (SELECT COUNT(*) FROM ${staging.affiliations}) <> 0
       OR (SELECT COUNT(*) FROM ${staging.finance}) <> ${finance.length} THEN
        RAISE EXCEPTION 'Production release staging setup count drift';
    END IF;
END
$$;
...
`;
  ...
  const claimLoadSql = claimParts.map((part) => {
    expectedClaimCount += part.length;
    return loadMigration(staging.claims, 'claims', part, expectedClaimCount, delimiter);
  });
  const affiliationLoadSql = affiliationParts.map((part) => {
    expectedAffiliationCount += part.length;
    return loadMigration(
      staging.affiliations,
      'affiliations',
      part,
      expectedAffiliationCount,
      delimiter,
    );
  });
...
`;
  ...
  const migrations = [setupSql, ...claimLoadSql, ...affiliationLoadSql, sql];
  assert(migrations.length === outputFiles.length, 'production release migration count drift');
  const outputPaths = outputFiles.map((file) => path.join(migrationDirectory, file));
  migrations.forEach((migration, index) => fs.writeFileSync(outputPaths[index], migration));
