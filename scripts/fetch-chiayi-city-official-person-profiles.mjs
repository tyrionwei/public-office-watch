...
const defaultOutputPath = path.join(repoRoot, 'data-sources', 'chiayi-city-official-person-profiles.seed.json');
const rawArchiveDir = path.join(repoRoot, 'local-data', 'raw', 'local', 'chiayi-city', 'official-person-profiles', 'current');
const fetchedAt = new Date().toISOString();

const councilSourceId = 'chiayi-city-council-current-councilors';
const councilSourceName = '嘉義市議會：本屆議員';
const councilPageRows = [
  {
    url: 'https://www.cycc.gov.tw/web/UnitStaff_New/Default.aspx?c0=3716&d0=3680&p0=0',
    districtLabel: '嘉義市第一選區',
  },
  {
    url: 'https://www.cycc.gov.tw/web/UnitStaff_New/Default.aspx?c0=3716&d0=3680&p0=1',
    districtLabel: '嘉義市第二選區',
  },
];

const govSourceId = 'chiayi-city-government-leaders';
const govSourceName = '嘉義市政府：市長';
const govLeaderRows = [
  { url: 'https://www.chiayi.gov.tw/cp.aspx?n=446', name: '黃敏惠', title: '市長', roleOrigin: 'elected', elected: true },
];
...
function archiveRawPage(url, response, bodyText) {
  ...
  sources.push({
    title: 'Chiayi City official person profile source page',
    sourceUrl: url,
    fetchedAt,
    status: response.status,
    ok: response.ok,
...
function scoreMatch(row, person) {
  ...
  if (row.sourceId === govSourceId && String(row.position ?? '').includes('市政府') && String(person.position ?? '').includes('市')) {
    score += 10;
    reasons.push('city government role matched');
  }
...
function matchPerson(row, peopleByName) {
  ...
  return {
    person: best.person,
    method: row.sourceId === govSourceId ? 'chiayi_city_government_profile_match' : 'chiayi_city_council_profile_match',
    score: best.score,
...
function parseCouncilRows(html, pageRow) {
  const rows = [];
  const itemPattern = /href\s*=\s*['"]([^'"]*Default2\.aspx\?c0=3716&p0=\d+)['"][^>]*title\s*=\s*['"]([^'"]+)['"][^>]*>([^<]+)<\/a>/giu;

  for (const match of html.matchAll(itemPattern)) {
    const profileUrl = new URL(decodeHtml(match[1]), pageRow.url).toString();
    const originalLabel = cleanInlineText(match[2] || match[3]);
    ...
    rows.push({
      ...
      externalId: 'current-councilor-' + hashId([pageRow.url, pageRow.districtLabel, name].join('|')),
      name,
      gender: 'unknown',
      party: '',
      position: '嘉義市議員',
...
function parseGovLeaderProfile(html, row) {
  ...
  const name = row.name;
  const education = fieldBetweenAny(content, ['學歷'], ['經歷', '現任', '網站功能']);
  const experience = fieldBetweenAny(content, ['經歷'], ['現任', '網站功能']);
...
async function main() {
  if (!anonKey) {
    throw new Error('Set SUPABASE_ANON_KEY for Chiayi City official person profile enrichment.');
  }
  ...
  const output = {
    schemaVersion: 1,
    name: 'chiayi-city-official-person-profiles',
    updatedAt: new Date().toISOString().slice(0, 10),
    notes: 'Chiayi City-specific official parser. Council rows cover current councilors from the two official Chiayi City Council district pages. City government rows cover the official Chiayi City Government mayor profile page.',
    sources: [
      ...councilPageRows.map((row) => ({ id: councilSourceId, name: `${councilSourceName}：${row.districtLabel}`, url: row.url })),
      { id: govSourceId, name: govSourceName, url: 'https://www.chiayi.gov.tw/cp.aspx?n=446' },
    ],
    summary,
...
main().catch((error) => {
  ...
  console.error(`Chiayi City official person profile enrichment failed: ${message}`);
  process.exit(1);
});
