...
const defaultOutputPath = path.join(repoRoot, 'data-sources', 'hsinchu-county-official-person-profiles.seed.json');
const rawArchiveDir = path.join(repoRoot, 'local-data', 'raw', 'local', 'hsinchu-county', 'official-person-profiles', 'current');
const fetchedAt = new Date().toISOString();

const councilSourceId = 'hsinchu-county-council-current-councilors';
const councilSourceName = '新竹縣議會：現任議員';
const councilDistrictRows = [
  ...
  ['36', '第四選區：關西鎮：', '關西鎮'],
  ['37', '第五選區：新埔鎮：', '新埔鎮'],
  ['38', '第六選區：橫山鄉、尖石鄉：', '橫山鄉、尖石鄉'],
  ['39', '第七選區：芎林鄉：', '芎林鄉'],
  ['40', '第八選區：竹東鎮、五峰鄉：', '竹東鎮、五峰鄉'],
  ['41', '第九選區：寶山鄉：', '寶山鄉'],
  ['42', '第十選區：北埔鄉、峨眉鄉：', '北埔鄉、峨眉鄉'],
  ['43', '第十一選區：平地原住民：', '平地原住民'],
  ['44', '第十二選區：尖石區：', '尖石區'],
  ['45', '第十三選區：五峰區：', '五峰區'],
].map(([code, label, district]) => ({
  code,
  label,
  district,
  url: `https://www.hcc.gov.tw/member-content?program=190&C=${code}`,
}));

const govSourceId = 'hsinchu-county-government-leaders';
const govSourceName = '新竹縣政府：縣長、副縣長、秘書長與副秘書長';
const govLeaderRows = [
  {
    url: 'https://www.hsinchu.gov.tw/cp.aspx?n=134',
    name: '楊文科',
    title: '縣長',
    roleOrigin: 'elected',
    elected: true,
  ...
  {
    url: 'https://www.hsinchu.gov.tw/cp.aspx?n=135',
    name: '陳見賢',
    title: '副縣長',
  ...
  {
    url: 'https://www.hsinchu.gov.tw/cp.aspx?n=3506',
    title: '副縣長',
    roleOrigin: 'appointed',
    elected: false,
  },
  {
    url: 'https://www.hsinchu.gov.tw/cp.aspx?n=136',
    title: '秘書長',
    roleOrigin: 'appointed',
    elected: false,
  },
  {
    url: 'https://www.hsinchu.gov.tw/cp.aspx?n=3504',
    title: '副秘書長',
    roleOrigin: 'appointed',
    elected: false,
  },
...
function archiveRawPage(url, response, bodyText) {
  ...
  sources.push({
    title: 'Hsinchu County official person profile source page',
    sourceUrl: url,
    fetchedAt,
    status: response.status,
    ok: response.ok,
...
function matchPerson(row, peopleByName) {
  ...
  return {
    person: best.person,
    method: row.sourceId === govSourceId ? 'hsinchu_county_government_profile_match' : 'hsinchu_county_council_profile_match',
    score: best.score,
...
function extractGovLeaderName(content, row) {
  if (row.name) return row.name;

  const patterns = [
    new RegExp(row.title + '\\s+([\\p{Script=Han}]{2,4})\\s+學歷', 'u'),
    new RegExp(row.title + '\\s+([\\p{Script=Han}]{2,4})\\s+經歷', 'u'),
    new RegExp(row.title + '\\s*([\\p{Script=Han}]{2,4})', 'u'),
  ];

  for (const pattern of patterns) {
    const name = content.match(pattern)?.[1] ?? '';
    if (name && !/介紹|首頁|專區/.test(name)) return name;
  }
...
function parseGovLeaderProfile(html, row) {
  const content = cleanInlineText(html);
  const name = extractGovLeaderName(content, row);
  const education = fieldBetweenAny(content, ['學歷'], ['經歷', '榮譽', '回上一頁', '回最上面']);
  const experience = fieldBetweenAny(content, ['經歷'], ['榮譽', '回上一頁', '回最上面']);

  if (!name || /介紹|首頁|專區/.test(name)) {
...
async function main() {
  if (!anonKey) {
    throw new Error('Set SUPABASE_ANON_KEY for Hsinchu County official person profile enrichment.');
  }
  ...
  const output = {
    schemaVersion: 1,
    name: 'hsinchu-county-official-person-profiles',
    updatedAt: new Date().toISOString().slice(0, 10),
    notes: 'Hsinchu County-specific official parser. Council rows cover current councilors by district from official Hsinchu County Council pages. County government rows cover leader profile pages that expose names on official Hsinchu County Government pages; unresolved leader pages are retained as skipped rows.',
    sources: [
      { id: councilSourceId, name: councilSourceName, url: 'https://www.hcc.gov.tw/member?lang=&program=190' },
      { id: govSourceId, name: govSourceName, url: 'https://www.hsinchu.gov.tw/cp.aspx?n=134' },
    ],
    summary,
...
main().catch((error) => {
  ...
  console.error(`Hsinchu County official person profile enrichment failed: ${message}`);
  process.exit(1);
});
