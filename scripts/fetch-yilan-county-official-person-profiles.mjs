...
const defaultOutputPath = path.join(repoRoot, 'data-sources', 'yilan-county-official-person-profiles.seed.json');
const rawArchiveDir = path.join(repoRoot, 'local-data', 'raw', 'local', 'yilan-county', 'official-person-profiles', 'current');
const fetchedAt = new Date().toISOString();

const councilSourceId = 'yilan-county-council-current-councilors';
const councilSourceName = '宜蘭縣議會：第20屆議員便民服務';
const councilListUrls = [{
  url: 'https://www.ilcc.gov.tw/Html/H_05/H_05.asp',
}];
const correctedCouncilDistrictNames = new Map([
  ['11', '平地原住民'],
  ['12', '山地原住民'],
  ['13', '山地原住民'],
]);

const govSourceId = 'yilan-county-government-leaders';
const govSourceName = '宜蘭縣政府：代理縣長、秘書長與局處首長';
const govLeaderRows = [
  {
    url: 'https://www.e-land.gov.tw/cp.aspx?n=2700',
    name: '林茂盛',
    title: '代理縣長',
  ...
  {
    url: 'https://www.e-land.gov.tw/cp.aspx?n=2701',
    name: '吳志宏',
    title: '秘書長',
    roleOrigin: 'appointed',
    elected: false,
  ...
  {
    url: 'https://www.e-land.gov.tw/cp.aspx?n=2704',
    title: '各局處首長',
    roleOrigin: 'appointed',
    elected: false,
    listPage: true,
...
function partyFromCouncilCode(value) {
  const code = cleanInlineText(value).toUpperCase();
  if (code === 'KMT') return '中國國民黨';
  if (code === 'DPP' || code === 'DDP') return '民主進步黨';
  return '無黨籍';
}
...
function archiveRawPage(url, response, bodyText) {
  ...
  sources.push({
    title: 'Yilan official person profile source page',
    sourceUrl: url,
    fetchedAt,
    status: response.status,
    ok: response.ok,
...
function matchPerson(row, peopleByName) {
  ...
  return {
    person: best.person,
    method: row.sourceId === govSourceId ? 'yilan_county_government_profile_match' : 'yilan_county_council_profile_match',
    score: best.score,
...
function parseCouncilRows(html, listRow) {
  ...
  for (const match of html.matchAll(/summary=["']第(\d+)選區\(([^)"']+)\)/giu)) {
    districtNames.set(match[1], cleanInlineText(match[2]));
  }

  const linkPattern = /<a\s+[^>]*href=["']([^"']*PBrowse\.aspx\?Sys_id=PMZ([^"']+))["'][^>]*title=["'][^"']*議員相關資料["'][^>]*>([\s\S]*?)<\/a>/giu;
  ...
  for (const match of html.matchAll(linkPattern)) {
    ...
    rows.push({
      ...
      sourceUrl,
      externalId: `current-councilor-${externalCode}`,
      name,
      gender: 'unknown',
      party: partyFromCouncilCode(rawPartyCode),
      position: '宜蘭縣議員',
      district: districtName ? `宜蘭縣第${districtNumber}選區（${districtName}）` : `宜蘭縣第${districtNumber}選區`,
      education: '',
      experience: '',
      sourcePayload: {
        profileHref: href,
        ...
        electoralDistrictName: districtName,
        rawPartyCode: cleanInlineText(rawPartyCode),
        roleOrigin: 'elected',
        elected: true,
...
async function fetchCouncilProfiles() {
  const parsedRows = await mapLimit(councilListUrls, 1, async (row) => {
    try {
...
function parseGovLeaderProfile(html, row) {
  ...
  const content = text;
  const name = row.name ??
    content.match(new RegExp('宜蘭縣政府' + row.title + '\n([\p{Script=Han}]{2,4})', 'u'))?.[1] ??
    '';
  const education = fieldBetweenAny(content, ['學歷'], ['經歷', '更新日期', '瀏覽人次', '回上一頁']);
  const experience = fieldBetweenAny(content, ['經歷'], ['更新日期', '瀏覽人次', '回上一頁']);
...
async function main() {
  if (!anonKey) {
    throw new Error('Set SUPABASE_ANON_KEY for Yilan County official person profile enrichment.');
  }
  ...
  const output = {
    schemaVersion: 1,
    name: 'yilan-county-official-person-profiles',
    updatedAt: new Date().toISOString().slice(0, 10),
    notes: 'Yilan County-specific official parser. County council rows come from the official 20th councilor service page and currently include name, district, party code, and profile link metadata from the list page. County government rows cover acting county magistrate, secretary-general, and agency heads from official Yilan County Government pages.',
    sources: [
      { id: councilSourceId, name: councilSourceName, url: 'https://www.ilcc.gov.tw/' },
      { id: govSourceId, name: govSourceName, url: 'https://www.e-land.gov.tw/cp.aspx?n=2700' },
    ],
    summary,
...
main().catch((error) => {
  ...
  console.error(`Yilan County official person profile enrichment failed: ${message}`);
  process.exit(1);
});
