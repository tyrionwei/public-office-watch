...
candidate_pairs AS (
    ...
    WHERE NOT EXISTS (
      ...
      AND NOT EXISTS (
        ...
        WHERE (
            existing_claim.source_person_id = sp.id
            OR existing_claim.claim_json->>'sourcePersonKey' = sp.source_person_key
        )
...
ranked_source_people AS (
    SELECT
        ...
        sp.updated_at
    FROM source_people sp
    JOIN candidate_pairs ON candidate_pairs.source_person_id = sp.id
