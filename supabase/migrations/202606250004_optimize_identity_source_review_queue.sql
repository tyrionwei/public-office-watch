CREATE INDEX IF NOT EXISTS idx_person_claims_review_source_person
ON person_claims (source_person_id, review_status)
WHERE person_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_person_claims_review_source_person_key
ON person_claims ((claim_json->>'sourcePersonKey'), review_status)
WHERE person_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_source_people_review_candidates
ON source_people (source_type, normalized_name);

CREATE INDEX IF NOT EXISTS idx_person_identity_matches_source_status
ON person_identity_matches (source_person_id, match_status);

CREATE INDEX IF NOT EXISTS idx_people_public_normalized_name
ON people ((lower(regexp_replace(replace(name, '臺', '台'), E'\\s+', '', 'g'))))
