-- Treat the live 11th Legislative Yuan roster as an explicit current-office source.
UPDATE source_people
SET
    source_payload = source_payload || jsonb_build_object(
        'isCurrent', TRUE,
        'term', COALESCE(source_payload ->> 'term', '11')
    ),
    updated_at = NOW()
WHERE source_id = 'ly-current-legislators'
  AND source_type = 'official_officeholder'
  AND election_year = 2024
  AND position LIKE '%第11屆立法委員%';
...
WITH leadership (
    ...
    VALUES
        (
            'ly-current-legislators:ly-leadership-11-president',
            '立法院全球資訊網：院長',
            'https://www.ly.gov.tw/Pages/Detail.aspx?nodeid=120&pid=1',
            '韓國瑜',
            '韓國瑜',
            '立法院院長（第11屆立法委員）',
            'ly-leadership-11-president',
            jsonb_build_object(
                'isCurrent', TRUE,
                'term', '11',
        ...
        (
            'ly-current-legislators:ly-leadership-11-vice-president',
            '立法院全球資訊網：副院長',
            'https://www.ly.gov.tw/Pages/Detail.aspx?nodeid=121&pid=2',
            '江啟臣',
            '江啟臣',
            '立法院副院長（第11屆立法委員）',
            'ly-leadership-11-vice-president',
            jsonb_build_object(
                'isCurrent', TRUE,
                'term', '11',
                'leadershipRole', 'vice_president',
                'startDate', '2024-02-01'
            )
        )
)
...
SELECT
    leadership.source_person_key,
    'official_officeholder',
    'ly-current-legislators',
    leadership.source_name,
    leadership.source_url,
    leadership.raw_name,
    leadership.normalized_name,
    'male',
    leadership.position,
    'legislator',
    2024,
    leadership.external_person_id,
    leadership.external_person_id,
    leadership.source_payload,
    'A',
    TRUE,
    NOW()
FROM leadership
...
WITH leadership_map (leadership_key, roster_key) AS (
    VALUES
        (
            'ly-current-legislators:ly-leadership-11-president',
            'ly-current-legislators:ly-legislator-11-110106'
        ),
        (
            'ly-current-legislators:ly-leadership-11-vice-president',
            'ly-current-legislators:ly-legislator-11-110009'
        )
),
resolved AS (
    SELECT
        leadership_source.id AS source_person_id,
        pcm.canonical_person_id
    FROM leadership_map
    JOIN source_people leadership_source
      ON leadership_source.source_person_key = leadership_map.leadership_key
    JOIN source_people roster_source
      ON roster_source.source_person_key = leadership_map.roster_key
    JOIN person_identity_matches roster_match
      ON roster_match.source_person_id = roster_source.id
     AND roster_match.match_status IN ('auto_matched', 'probable_match')
    JOIN person_canonical_map pcm ON pcm.person_id = roster_match.person_id
)
...
SELECT
    resolved.source_person_id,
    resolved.canonical_person_id,
    'auto_matched',
    100,
    'official_external_id',
    'Official Legislative Yuan leadership record matched through the verified current-legislator roster identity.',
    jsonb_build_object('version', 'ly-leadership-v1'),
    'system:ly-leadership',
    NOW(),
    NOW()
FROM resolved
...
SELECT
    leadership_source.source_person_key || ':office',
    leadership_match.person_id,
    leadership_source.id,
    'office',
    leadership_source.position,
    leadership_source.source_payload || jsonb_build_object(
        'sourcePersonKey', leadership_source.source_person_key
    ),
    'A',
    'verified',
    'public',
    leadership_source.source_name,
    leadership_source.source_url,
    TIMESTAMPTZ '2024-02-01 09:35:00+00',
    TRUE,
    100,
    'ly-leadership-v1',
    jsonb_build_array('Official Legislative Yuan leadership page'),
    NOW(),
    NOW()
FROM source_people leadership_source
JOIN person_identity_matches leadership_match
  ON leadership_match.source_person_id = leadership_source.id
 AND leadership_match.match_status IN ('auto_matched', 'probable_match')
WHERE leadership_source.source_person_key IN (
    'ly-current-legislators:ly-leadership-11-president',
    'ly-current-legislators:ly-leadership-11-vice-president'
)
