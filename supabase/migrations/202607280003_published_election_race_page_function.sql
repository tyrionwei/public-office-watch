...
GRANT EXECUTE ON FUNCTION published.election_race_page(TEXT, UUID[], TEXT[], TEXT, INTEGER, INTEGER)
TO service_role;

COMMENT ON FUNCTION published.election_race_page(TEXT, UUID[], TEXT[], TEXT, INTEGER, INTEGER) IS
    'Bounded event race page with exact totals and deterministic ordering over requested canonical elections.';
