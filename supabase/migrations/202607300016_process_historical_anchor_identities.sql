...
BEGIN
    WITH source_normalized AS MATERIALIZED (
        SELECT
            ...
            CASE
                WHEN replace(source.district, '臺', '台') ~ '^(台北縣|新北市)' THEN '新北市'
                WHEN replace(source.district, '臺', '台') ~ '^(桃園縣|桃園市)' THEN '桃園市'
                WHEN replace(source.district, '臺', '台') ~ '^(台中縣|台中市)' THEN '台中市'
                WHEN replace(source.district, '臺', '台') ~ '^(台南縣|台南市)' THEN '台南市'
                WHEN replace(source.district, '臺', '台') ~ '^(高雄縣|高雄市)' THEN '高雄市'
                ELSE substring(replace(source.district, '臺', '台') FROM '^(.+?[縣市])')
    ...
    public_people_normalized AS MATERIALIZED (
        SELECT
            ...
            person.gender,
            CASE
                WHEN replace(person.district, '臺', '台') ~ '^(台北縣|新北市)' THEN '新北市'
                WHEN replace(person.district, '臺', '台') ~ '^(桃園縣|桃園市)' THEN '桃園市'
                WHEN replace(person.district, '臺', '台') ~ '^(台中縣|台中市)' THEN '台中市'
                WHEN replace(person.district, '臺', '台') ~ '^(台南縣|台南市)' THEN '台南市'
                WHEN replace(person.district, '臺', '台') ~ '^(高雄縣|高雄市)' THEN '高雄市'
                ELSE substring(replace(person.district, '臺', '台') FROM '^(.+?[縣市])')
            END AS canonical_geography,
            CASE
    ...
    safe_source_groups AS (
        SELECT canonical_name, gender, canonical_geography, canonical_role
        ...
        GROUP BY canonical_name, gender, canonical_geography, canonical_role
    ...
    upserted_matches AS (
        ...
        SELECT
            ...
            'auto_matched',
            99,
            'official_historical_anchor_propagation_v1',
            'auto-approved: a unique official identity anchor has the same canonical name, gender, geography, and role; election years are non-duplicated',
            jsonb_build_object(
                'version', 'official-historical-anchor-propagation-v1',
            ...
            'system:official-historical-anchor-propagation-v1',
            NOW(),
            NOW()
...
REVOKE ALL ON FUNCTION public.process_historical_anchor_identities() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.process_historical_anchor_identities() FROM anon;
REVOKE ALL ON FUNCTION public.process_historical_anchor_identities() FROM authenticated;
GRANT EXECUTE ON FUNCTION public.process_historical_anchor_identities() TO service_role;
