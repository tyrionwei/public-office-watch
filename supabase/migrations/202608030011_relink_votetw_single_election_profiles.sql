...
-- only election record must uniquely match that target's published VoteTW
-- historical candidate record. Published values remain capped at confidence C.
CREATE TEMP TABLE _votetw_single_election_profiles AS
SELECT DISTINCT
    CASE
        WHEN claim.claim_json->'identityResolution'->>'version' =
             'votetw-single-election-source-anchor-v1'
            THEN (
...
CREATE TEMP TABLE _votetw_single_election_records AS
SELECT DISTINCT
...
FROM _votetw_single_election_profiles profile
...
CREATE TEMP TABLE _votetw_single_election_matches AS
SELECT DISTINCT
...
FROM _votetw_single_election_records source
...
CREATE TEMP TABLE _votetw_single_election_summary AS
SELECT
    profile.*,
    (
        SELECT COUNT(DISTINCT record.source_election)
        FROM _votetw_single_election_records record
...
FROM _votetw_single_election_profiles profile
LEFT JOIN _votetw_single_election_matches match
...
CREATE TEMP TABLE _votetw_single_election_relinks AS
SELECT summary.*
FROM _votetw_single_election_summary summary
...
BEGIN
    IF (
        SELECT COUNT(*) FROM _votetw_single_election_relinks
    ) <> 255 THEN
        RAISE EXCEPTION
            'VoteTW single-election relink count drifted from 255';
    END IF;

    IF (
        SELECT COUNT(*)
        FROM _votetw_single_election_relinks relink
        ...
        RAISE EXCEPTION
            'VoteTW single-election profile claim count drifted from 1533';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'VoteTW single-election relink would duplicate a platform claim';
    END IF;
END;
$$;
...
SET
    ...
    claim_json = claim.claim_json ||
        jsonb_build_object(
            'identityResolution',
            jsonb_build_object(
                'version', 'votetw-single-election-source-anchor-v1',
                'originalPersonId', relink.owner_person_id,
                'targetPersonId', relink.target_person_id,
                'sourceUrl', relink.source_url,
                'reason', 'exact VoteTW person source URL and unique single-election candidate match',
                'reviewedAt', NOW()
            )
        ),
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        CASE
            ...
            ELSE jsonb_build_array(
                jsonb_build_object(
                    'version', 'votetw-single-election-source-anchor-v1',
                    'reason', 'VoteTW profile relinked to the same-name person whose exact source URL and single election record both match',
                    'reviewedAt', NOW()
                )
            )
        END,
...
CREATE TEMP TABLE _votetw_single_election_claims AS
SELECT
...
WHERE claim.source_name = 'VoteTW'
  ...
  AND (
      ...
      OR claim.scoring_version IN (
          'votetw-single-election-claim-rereview-v1',
          'votetw-single-election-claim-conflict-v1'
      )
  );

CREATE TEMP TABLE _votetw_single_election_decisions AS
WITH classified AS (
    SELECT
        candidate.*,
        person.gender,
    ...
    FROM _votetw_single_election_claims candidate
...
BEGIN
    IF (
        SELECT COUNT(*) FROM _votetw_single_election_decisions
    ) <> 1236 THEN
        RAISE EXCEPTION
            'VoteTW single-election review count drifted from 1236';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW single-election publish count drifted from 1232';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW single-election archive count drifted from 3';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW single-election hold count drifted from 1';
    END IF;
END;
$$;
...
SET
    ...
    visibility = 'public',
    is_public = TRUE,
    scoring_version = 'votetw-single-election-claim-rereview-v1',
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        jsonb_build_array(
            jsonb_build_object(
                'version', 'votetw-single-election-claim-rereview-v1',
                'reason', 'VoteTW claim is attached to the person with the exact source URL and a unique matching single election record',
                'reviewedAt', NOW()
            )
        ),
    auto_reviewed_at = COALESCE(claim.auto_reviewed_at, NOW()),
    updated_at = NOW()
FROM _votetw_single_election_decisions decision
...
UPDATE person_claims claim
SET
    scoring_version = 'votetw-single-election-claim-conflict-v1',
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        jsonb_build_array(
            jsonb_build_object(
                'version', 'votetw-single-election-claim-conflict-v1',
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'VoteTW single-election relink state mismatch';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'VoteTW single-election claim state mismatch';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW single-election legal claim state drifted from 1';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW single-election platform claim state drifted from 2';
    END IF;
END;
$$;
...
DROP TABLE _votetw_single_election_decisions;
DROP TABLE _votetw_single_election_claims;
DROP TABLE _votetw_single_election_relinks;
DROP TABLE _verified_non_votetw_claims;
DROP TABLE _votetw_single_election_summary;
DROP TABLE _votetw_single_election_matches;
DROP TABLE _votetw_single_election_records;
DROP TABLE _votetw_single_election_profiles;
