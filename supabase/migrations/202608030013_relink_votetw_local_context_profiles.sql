...
-- person's source URL must be the exact VoteTW profile URL, while the source
-- year, complete local race title and party must uniquely match the target's
-- published VoteTW historical candidate record. Published values remain
-- capped at confidence C.
CREATE TEMP TABLE _votetw_local_context_profiles AS
SELECT DISTINCT
    CASE
        WHEN claim.claim_json->'identityResolution'->>'version' =
             'votetw-local-context-source-anchor-v1'
            THEN (
...
CREATE TEMP TABLE _votetw_local_context_records AS
SELECT DISTINCT
...
FROM _votetw_local_context_profiles profile
...
CREATE TEMP TABLE _votetw_local_context_matches AS
SELECT DISTINCT
...
FROM _votetw_local_context_records source
...
WHERE SUBSTRING(source.source_election FROM 1 FOR 4) ~ '^[0-9]{4}$'
  ...
  AND (
      pg_temp._votetw_single_norm_text(source.source_election) LIKE '%' ||
          pg_temp._votetw_single_norm_text(election.name) || '%'
      OR pg_temp._votetw_single_norm_text(source.source_election) LIKE '%' ||
          pg_temp._votetw_single_norm_text(race.title) || '%'
  )
  AND (
...
CREATE TEMP TABLE _votetw_local_context_summary AS
SELECT
    profile.*,
    (
        SELECT COUNT(DISTINCT record.source_election)
        FROM _votetw_local_context_records record
...
FROM _votetw_local_context_profiles profile
LEFT JOIN _votetw_local_context_matches match
...
CREATE TEMP TABLE _votetw_local_context_relinks AS
SELECT summary.*
FROM _votetw_local_context_summary summary
JOIN people target ON target.id = summary.target_person_id
WHERE summary.source_record_count = 1
...
BEGIN
    IF (
        SELECT COUNT(*) FROM _votetw_local_context_relinks
    ) <> 184 THEN
        RAISE EXCEPTION
            'VoteTW local-context relink count drifted from 184';
    END IF;

    IF (
        SELECT COUNT(*)
        FROM _votetw_local_context_relinks relink
        ...
        RAISE EXCEPTION
            'VoteTW local-context profile claim count drifted from 1104';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'VoteTW local-context relink would duplicate a platform claim';
    END IF;
END;
$$;
...
SET
    ...
    claim_json = claim.claim_json ||
        jsonb_build_object(
            'identityResolution',
            jsonb_build_object(
                'version', 'votetw-local-context-source-anchor-v1',
                'originalPersonId', relink.owner_person_id,
                'targetPersonId', relink.target_person_id,
                'sourceUrl', relink.source_url,
                'reason', 'exact VoteTW person source URL and unique year, complete local race title and party match',
                'reviewedAt', NOW()
            )
        ),
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        CASE
            ...
            ELSE jsonb_build_array(
                jsonb_build_object(
                    'version', 'votetw-local-context-source-anchor-v1',
                    'reason', 'VoteTW profile relinked to the same-name person whose exact source URL and local election context match',
                    'reviewedAt', NOW()
                )
            )
        END,
...
CREATE TEMP TABLE _votetw_local_context_claims AS
SELECT
...
WHERE claim.source_name = 'VoteTW'
  ...
  AND (
      ...
      OR claim.scoring_version IN (
          'votetw-local-context-claim-rereview-v1',
          'votetw-local-context-claim-conflict-v1'
      )
  );

CREATE TEMP TABLE _votetw_local_context_decisions AS
WITH classified AS (
    SELECT
        candidate.*,
        person.gender,
    ...
    FROM _votetw_local_context_claims candidate
...
BEGIN
    IF (
        SELECT COUNT(*) FROM _votetw_local_context_decisions
    ) <> 900 THEN
        RAISE EXCEPTION
            'VoteTW local-context review count drifted from 900';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW local-context publish count drifted from 880';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW local-context archive count drifted from 18';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW local-context hold count drifted from 2';
    END IF;
END;
$$;
...
SET
    ...
    visibility = 'public',
    is_public = TRUE,
    scoring_version = 'votetw-local-context-claim-rereview-v1',
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        jsonb_build_array(
            jsonb_build_object(
                'version', 'votetw-local-context-claim-rereview-v1',
                'reason', 'VoteTW claim is attached to the person with the exact source URL and a unique matching local election context',
                'reviewedAt', NOW()
            )
        ),
    auto_reviewed_at = COALESCE(claim.auto_reviewed_at, NOW()),
    updated_at = NOW()
FROM _votetw_local_context_decisions decision
...
UPDATE person_claims claim
SET
    scoring_version = 'votetw-local-context-claim-conflict-v1',
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        jsonb_build_array(
            jsonb_build_object(
                'version', 'votetw-local-context-claim-conflict-v1',
                'reason', 'VoteTW education level differs from the target person current education value',
                'reviewedAt', NOW()
            )
        ),
    updated_at = NOW()
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'VoteTW local-context relink state mismatch';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'VoteTW local-context claim state mismatch';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW local-context legal claim state drifted from 0';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW local-context platform claim state drifted from 0';
    END IF;
END;
$$;
...
DROP TABLE _votetw_local_context_decisions;
DROP TABLE _votetw_local_context_claims;
DROP TABLE _votetw_local_context_relinks;
DROP TABLE _verified_non_votetw_claims;
DROP TABLE _votetw_local_context_summary;
DROP TABLE _votetw_local_context_matches;
DROP TABLE _votetw_local_context_records;
DROP TABLE _votetw_local_context_profiles;
