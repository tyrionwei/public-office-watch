...
-- Review already-linked VoteTW profiles when multiple source election records
-- have exactly one locally matchable person, which is the current person. Local
...
CREATE TEMP TABLE _votetw_existing_multi_context_profiles AS
SELECT DISTINCT
    claim.person_id,
    person.name,
    claim.source_url,
...
WHERE claim.source_name = 'VoteTW'
  ...
  AND (
      ...
      OR claim.scoring_version IN (
          'votetw-existing-multi-context-anchor-v1',
...
CREATE TEMP TABLE _votetw_existing_multi_context_records AS
SELECT DISTINCT
...
FROM _votetw_existing_multi_context_profiles pending
...
CREATE TEMP TABLE _votetw_existing_multi_context_matches AS
SELECT DISTINCT
...
FROM _votetw_existing_multi_context_records source
...
CREATE TEMP TABLE _votetw_existing_multi_context_summary AS
SELECT
    pending.*,
...
FROM _votetw_existing_multi_context_profiles pending
LEFT JOIN _votetw_existing_multi_context_matches match
...
CREATE TEMP TABLE _votetw_existing_multi_context_eligible AS
SELECT summary.*
FROM _votetw_existing_multi_context_summary summary
...
WHERE summary.matched_person_count = 1
  ...
  AND (
      ...
      FROM _votetw_existing_multi_context_records record
...
CREATE INDEX _verified_non_votetw_claims_existing_multi_context_idx
...
CREATE TEMP TABLE _votetw_existing_multi_context_claims AS
SELECT
    claim.id AS claim_id,
    claim.person_id,
    claim.claim_type,
    claim.claim_value
FROM _votetw_existing_multi_context_eligible eligible
...
CREATE TEMP TABLE _votetw_existing_multi_context_decisions AS
WITH classified AS (
    SELECT
        candidate.*,
        person.gender,
    ...
    FROM _votetw_existing_multi_context_claims candidate
...
BEGIN
    IF (
        SELECT COUNT(*) FROM _votetw_existing_multi_context_eligible
    ) <> 6 THEN
        RAISE EXCEPTION
            'VoteTW existing multi-context profile count drifted from 6';
    END IF;

    IF (
        SELECT COUNT(*) FROM _votetw_existing_multi_context_decisions
    ) <> 14 THEN
        RAISE EXCEPTION
            'VoteTW existing multi-context claim count drifted from 14';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW existing multi-context publish count drifted from 9';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW existing multi-context archive count drifted from 0';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION
            'VoteTW existing multi-context hold count drifted from 5';
    END IF;
END;
$$;
...
SET
    ...
    visibility = 'public',
    is_public = TRUE,
    scoring_version = 'votetw-existing-multi-context-anchor-v1',
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        jsonb_build_array(
            jsonb_build_object(
                'version', 'votetw-existing-multi-context-anchor-v1',
                'reason', 'VoteTW profile and exact source URL uniquely match the current person through the locally matchable election history and exact source URL',
                'reviewedAt', NOW()
            )
        ),
    auto_reviewed_at = COALESCE(claim.auto_reviewed_at, NOW()),
    updated_at = NOW()
FROM _votetw_existing_multi_context_decisions decision
...
UPDATE person_claims claim
SET
    scoring_version = 'votetw-existing-multi-context-conflict-v1',
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        jsonb_build_array(
            jsonb_build_object(
                'version', 'votetw-existing-multi-context-conflict-v1',
                'reason',
...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'VoteTW existing multi-context review state mismatch';
    END IF;
END;
$$;
...
DROP TABLE _votetw_existing_multi_context_decisions;
DROP TABLE _votetw_existing_multi_context_claims;
DROP TABLE _verified_non_votetw_claims;
DROP TABLE _votetw_existing_multi_context_eligible;
DROP TABLE _votetw_existing_multi_context_summary;
DROP TABLE _votetw_existing_multi_context_matches;
DROP TABLE _votetw_existing_multi_context_records;
DROP TABLE _votetw_existing_multi_context_profiles;
