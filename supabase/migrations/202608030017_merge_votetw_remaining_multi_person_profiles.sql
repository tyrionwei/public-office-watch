...
-- Four birth-dated VoteTW profiles span multiple exact election records that
-- were imported as separate public people. The same-source profile consistency,
-- candidate history, and existing canonical chains support five conservative
-- merges. Chen Kuei-you's unlisted 2018 candidacy remains separate. These merge
-- decisions stay at C rather than being treated as independent corroboration.
CREATE TEMP TABLE _votetw_remaining_multi_person_merges (
...
INSERT INTO _votetw_remaining_multi_person_merges VALUES
    (
        'votetw-person-a0ff20825af55b9e',
        'votetw-person-4443440dd2bef778',
        '彭振芳',
        '1964-05-20',
        '2024年立法委員選舉臺中市第5選舉區（區域）',
        '司法改革黨',
        '2020年立法委員選舉臺中市第5選舉區（區域）',
        '臺灣前進黨'
    ),
    (
        'votetw-person-672c05b7ab62a672',
        'votetw-person-53d39c979957f7c0',
        '許願神',
        '1954-06-12',
        '2024年立法委員選舉花蓮縣選舉區（區域）',
        '無',
        '2022年花蓮縣花蓮市市民代表選舉花蓮市第3選舉區（區域）',
        '無'
    ),
    (
        'votetw-person-672c05b7ab62a672',
        'votetw-person-1d7410638ce799b4',
        '許願神',
        '1954-06-12',
        '2024年立法委員選舉花蓮縣選舉區（區域）',
        '無',
        '2020年立法委員選舉花蓮縣選舉區（區域）',
        '臺灣前進黨'
    ),
    (
        'votetw-person-80b5d97c4ef9a3b6',
        'votetw-person-cec919e94e466257',
        '錢宥均',
        '1981-06-17',
        '2022年南投縣仁愛鄉鄉長選舉',
        '無',
    ...
    (
        ...
        'votetw-person-28ef38816a3e1724',
        '陳癸佑',
        '1968-09-15',
        '2024年立法委員選舉南投縣第2選舉區（區域）',
        '無',
        '2020年立法委員選舉南投縣第2選舉區（區域）',
        '民主進步黨'
    );
...
BEGIN
    IF (
        SELECT COUNT(*)
        FROM _votetw_remaining_multi_person_merges input
        ...
        RAISE EXCEPTION
            'VoteTW multi-person cross-year identity evidence drifted from 5 pairs';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'VoteTW multi-person cross-year identity gained a conflicting merge decision';
    END IF;
END;
$$;
...
SELECT
    ...
    jsonb_build_object(
        'version', 'votetw-multi-person-cross-year-election-anchor-v1',
    ...
    'system:votetw-multi-person-cross-year-election-anchor-v1',
    NOW(),
    NOW()
FROM _votetw_remaining_multi_person_merges input
JOIN people canonical
...
SET
    ...
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        jsonb_build_array(
            jsonb_build_object(
                'version', 'votetw-multi-person-cross-year-election-anchor-v1',
                'reason', 'pending claim relinked after a verified VoteTW multi-person cross-year identity merge',
                'reviewedAt', NOW()
            )
        ),
    updated_at = NOW()
...
CREATE TEMP TABLE _votetw_remaining_multi_person_claim_candidates AS
SELECT DISTINCT
...
WHERE claim.source_name = 'VoteTW'
  ...
  AND (
      ...
      OR claim.scoring_version IN (
          'votetw-multi-person-cross-year-claim-rereview-v1',
          'votetw-multi-person-cross-year-claim-conflict-v1'
      )
  );
...
CREATE TEMP TABLE _votetw_remaining_multi_person_claim_decisions AS
WITH classified AS (
    SELECT
        candidate.*,
        person.gender,
    ...
    FROM _votetw_remaining_multi_person_claim_candidates candidate
...
DO $$
BEGIN
    IF (SELECT COUNT(*) FROM _votetw_remaining_multi_person_claim_decisions) <> 8 THEN
        RAISE EXCEPTION 'VoteTW multi-person cross-year claim total drifted from 8';
    END IF;

    IF (
        SELECT COUNT(*) FROM _votetw_remaining_multi_person_claim_decisions
        WHERE action = 'publish_c'
    ) <> 5 THEN
        RAISE EXCEPTION 'VoteTW multi-person cross-year claim publish count drifted from 5';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION 'VoteTW multi-person cross-year claim archive count drifted from 0';
    END IF;
    ...
    IF (
        ...
        RAISE EXCEPTION 'VoteTW multi-person cross-year claim hold count drifted from 3';
    END IF;
END;
$$;
...
SET
    ...
    visibility = 'public',
    is_public = TRUE,
    scoring_version = 'votetw-multi-person-cross-year-claim-rereview-v1',
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        jsonb_build_array(
            jsonb_build_object(
                'version', 'votetw-multi-person-cross-year-claim-rereview-v1',
    ...
    updated_at = NOW()
FROM _votetw_remaining_multi_person_claim_decisions decision
...
UPDATE person_claims claim
SET
    scoring_version = 'votetw-multi-person-cross-year-claim-conflict-v1',
    scoring_reasons = COALESCE(claim.scoring_reasons, '[]'::JSONB) ||
        jsonb_build_array(
            jsonb_build_object(
                'version', 'votetw-multi-person-cross-year-claim-conflict-v1',
...
BEGIN
    IF (
        ...
        RAISE EXCEPTION 'VoteTW multi-person cross-year merge state mismatch';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'VoteTW multi-person cross-year claim state mismatch';
    END IF;
END;
$$;
...
DROP TABLE _votetw_remaining_multi_person_claim_decisions;
DROP TABLE _verified_non_votetw_claim_types;
DROP TABLE _votetw_remaining_multi_person_claim_candidates;
DROP TABLE _votetw_remaining_multi_person_merges;
