...
verified_external_ids AS (
    SELECT DISTINCT
        ...
        lower(
            CASE
                WHEN pc.claim_value ~ '^current-councilor-[^:]+$'
                  AND NULLIF(TRIM(pc.claim_json->>'sourcePersonKey'), '') IS NOT NULL
                    THEN TRIM(pc.claim_json->>'sourcePersonKey')
                ELSE COALESCE(
                    pc.claim_value,
                    CASE
...
pair_candidates AS (
    ...
    JOIN public_people_base left_person ON left_person.id = left_ids.person_id
    JOIN public_people_base right_person ON right_person.id = right_ids.person_id
    WHERE NOT (
        left_person.birth_date IS NOT NULL
        AND right_person.birth_date IS NOT NULL
        AND left_person.birth_date <> right_person.birth_date
    )
      AND NOT (
    ...
    SELECT
        ...
        'same normalized name, gender, and birth date without shared external ID' AS reason,
        ...
        jsonb_build_object(
            ...
            'note', 'strong candidate, but not A-level without a shared verified external ID'
        ) AS evidence_json
...
AND NOT EXISTS (
    SELECT 1
    FROM person_canonical_map duplicate_map
    JOIN person_canonical_map canonical_map
      ON canonical_map.person_id = ranked_pairs.suggested_canonical_person_id
     AND canonical_map.canonical_person_id = duplicate_map.canonical_person_id
...
COMMENT ON VIEW person_duplicate_review_queue IS
    'Duplicate-person review queue using source-scoped councilor IDs and excluding people that already resolve to the same canonical identity.';
