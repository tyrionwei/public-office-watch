...
DO $$
BEGIN
    IF (SELECT COUNT(*) FROM _profile_claim_archive_targets) <> 928 THEN
        RAISE EXCEPTION 'Profile claim archive target drifted: expected 928, got %',
    ...
    IF (
        ...
        RAISE EXCEPTION 'Profile claim duplicate partition count drifted';
    END IF;
